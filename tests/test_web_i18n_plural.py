"""CI 디스커버리 shim — 실제 영문 복수형 표지 가드는 `web/tests/test_i18n_plural.py`.
...
(`test_web_linkedin_cards.py` 와 동형) — 수동 `__all__` 목록은 새 클래스를 빠뜨리면 그
테스트가 CI 에서 조용히 실행되지 않으므로 쓰지 않는다.
"""
import inspect
import pathlib
import sys
...
import test_i18n_plural as _web_i18n_plural  # noqa: E402

_EXPORTED = []
for _name, _obj in inspect.getmembers(_web_i18n_plural, inspect.isclass):
    if issubclass(_obj, unittest.TestCase) and _obj is not unittest.TestCase:
        globals()[_name] = _obj
        _EXPORTED.append(_name)


class WebI18nPluralShimCoverageTest(unittest.TestCase):
    """shim 자체의 회귀 가드 — web/tests/test_i18n_plural.py 가 정의한 TestCase 는 하나도
    빠짐없이 이 모듈로 넘어와야 한다(미실행 테스트 0)."""

    def test_every_web_testcase_is_reexported(self):
        src = (_WEB_TESTS / "test_i18n_plural.py").read_text(encoding="utf-8")
        defined = {ln.split("class ", 1)[1].split("(", 1)[0]
                   for ln in src.splitlines()
                   if ln.startswith("class ") and "unittest.TestCase" in ln}
        self.assertTrue(defined, "web/tests/test_i18n_plural.py 에서 TestCase 를 찾지 못함")
