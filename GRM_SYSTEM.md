...
| 전략 로드맵 단계 | 목표 | 현황 |
|---|---|---|
| M1(7월) 스키마+내부 백필 | grm-finding/v1 동결, Notion 일회성 익스포트, 이중 적재 | ✅ 완료(§4.4 M1~M14가 대체·확장) |
| M2(8월) 외부 백필(볼륨) | FDA 483 전수(~2,000건)·WL 수년치·MFDS GMP 과거 PDF, 3~5년치 데이터 | 🔲 **미착수** — 현재 라이브 46건뿐, 트렌드 주장의 전제(결정 사항 #5) 미충족 |
| M3(9월) 서빙+분석 대시보드 | 파셋검색·히트맵·업체별 지적 이력 페이지·실사관(inspector) 프로파일 | 🟡 **부분** — 파셋검색·분포 통계(M7)는 있음, 히트맵·업체이력·실사관 프로파일은 없음 |
| M4(10월) Copilot 에이전트+파일럿 | Copilot Studio 커스텀 커넥터 연동, Q&A 튜닝, 생산·품질·RA 3부서 파일럿, 효과 정량화 | 🔲 **미착수**(사내 M365 테넌트 Copilot Studio 권한 확인이 선결 — 로드맵 자체가 "미결"로 표시) |
| M5(11월~공모전) 패키징 | 효과 정리·데모 리허설·발표자료 | 🔲 미착수(M4 선행 필요) |

> 이 문서는 그동안 git에 커밋되지 않고 로컬 파일로만 존재해 유실 위험이 있었다(2026-07-09 `docs/`로 이관). 향후 착수 시 이 §6.4와 로드맵 문서 본문을 함께 갱신한다.
