...
직접 호출해 확인한 값이다**(2026-08-11, 발급 자격증명으로 실측). 함께 확인한 것:
`ProjectArea` 를 filters 에 넣으면 **413 invalid_filters** 로 거부된다(반환 전용 컬럼이라
클라이언트에서 걸러야 하는 이유가 이것이다), `rows=20000` 은 400 이고 5000 은 200,
`ProductType=Drugs` + 최근 7 회계연도로 좁히면 수신 11,104건 → GMP(`Drug Quality
Assurance`) **6,417건**(NAI 1,409 / VAI 4,055 / OAI 953)이 남는다.
※이 모듈의 유닛 테스트는 자격증명 없이 도는 오프라인 검증이라(응답 파싱·필터링·정규화·
적재 로직만) 위 계약을 재확인하지 않는다 — 계약이 깨지면 첫 `--dry-run` 이 에러 응답으로
즉시 드러낸다(필터가 틀리면 침묵하지 않는다).
