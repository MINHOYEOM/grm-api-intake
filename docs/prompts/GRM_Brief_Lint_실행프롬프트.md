# GRM Brief Lint — 발행 후 검증 실행프롬프트 (v1.1)
...
L11 출처 링크 근거(provenance, HARD FAIL — 2026-06-15 W24 회귀): 모든 카드 📰/📎 링크가
    ...
    **MFDS/nedrug 링크**가 본문에 있으면 FAIL — 특히 `mfds.go.kr/brd/*/view.do?seq=`(보도자료
    ...
    즉시 FAIL(엉뚱한 보도자료/오류 페이지를 가리킨 사고의 재발 차단). 비-MFDS 외부 링크가 근거
    없으면 WARN(검색 카드면 이번 run 에 실제 fetch 했는지 확인, 아니면 패턴 유추 의심).

[검사 방법]
- 페이지 본문을 한 번 fetch 해 전 카드를 순회하며 L1~L6, L10 을 채점한다.
- Intake DB 는 해당 주 handoff page 1건 + 그 rows 의 Status 만 조회한다 (broad search 금지).
- L11 은 코드 게이트 `brief_lint.lint_link_provenance(handoff_rows, 발행_markdown)` 로 기계 판정
  가능하다(handoff rows = 그 주 handoff page 본문 JSON 의 `rows[]`, 발행_markdown = 페이지 export).
  MCP 만으로 볼 때는 본문 mfds/nedrug 링크의 seq 를 handoff rows 의 official_url·card_scaffold 와
  대조한다. (Publish Lint 17 과 동일 불변식 — 작성 세션 / 독립 세션 이중 게이트.)
