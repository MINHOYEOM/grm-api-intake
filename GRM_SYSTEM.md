...
| P4-검증 | Notion Intake DB 에 `Modality` select 속성 생성 → `ENABLE_MODALITY_TAG=true`(워크플로우 wiring 포함) → 1~2주 라이브에서 화학합성/생물 카드 분류·제품군별 발행 그룹핑·거짓음성(무균오염·CCIT 등 누락) 점검 후 `feature/multi-modality` 머지 | 🔲 대기 |
