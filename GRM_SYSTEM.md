...
├─ brief_lint.py          # [URL전수검사] 발행물 출처 링크 근거(provenance) 하드 가드(순수)+발행 게이트 CLI(W1)+resolve&verify 엔진
