...
#
# [우리 쪽 어휘 확장 2026-07-27] 종전 정규식은 **소스를 탓하는 표현**(원문에 없다·미기재)만
# 잡았다. 그런데 2026-07-20 어휘 교정으로 그 표현이 "우리가 확보하지 못했다"로 바뀌었고,
# 스캔 483 을 OCR 로 복구한 뒤 그 서술이 **거짓이 된 카드 25장이 게이트를 그대로 통과했다**
# — 관찰 상세가 바로 아래 붙어 있는데 "관찰사항 본문을 확보하지 못했다"고 적힌 채 발행됐다
# (2026-07-27 실측: 07-27 24건·07-20 1건, 전부 key_facts 한 줄).
# 어휘를 정직하게 고치는 것과 그 서술이 여전히 참인지는 **별개 문제**다. 원문을 손에 쥔
# 카드에서는 주어가 소스든 우리든 부재 주장 자체가 거짓이므로 둘 다 막는다.
_FALSE_ABSENCE_RE = re.compile(
    r"(?:위반|관찰|지적|결함|조항|근거|사유|처분|상세|내용|본문)[^.\n]{0,24}?"
    r"(?:"
    r"(?:원문|본문|공개)[^.\n]{0,16}?"          # ← 12→16(실측 13자에 1글자 차이로 빠져나갔다)
...
def merge_fda483_disclosures(cards: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    #
    # [부재 어휘 재교정 2026-07-27] 그 다음 문구("개별 관찰 원문이 **제공되지 않아**")도 여전히
    # **소스가 안 줬다**는 단정이었고, 그것도 거짓이었다. 실측: 그 카드들의 원문 PDF 에는 관찰이
    # 멀쩡히 들어 있고 다만 **스캔 이미지**여서 우리가 못 읽었을 뿐이다(2026-07-26 실측 21건 —
    # 텍스트층은 마지막 장 FDA 정형 고지문뿐). 소스의 상태를 말하지 않고 **우리 상태만** 말한다.
    rep["summary"] = (
        f"FDA OII FOIA 전자열람실에 483 실사기록 {n}건이 공개됐다. 원문 PDF 는 공개돼 있으나 "
        "개별 관찰 본문을 자동으로 확보하지 못해 시설·실사일 목록만 정리했다.")
    rep["key_facts"] = [
        f"공개 건수: {n}건 (관찰 본문 자동 확보 실패 — 원문 PDF 는 공개돼 있음)",
        f"대표 시설: {rep_fac} 외 {n - 1}곳",
        "출처: FDA OII FOIA 전자열람실",
    ]
    rep["implication"] = (
        "관찰 본문을 확보하지 못한 실사기록 공개는 그 자체로 조치 신호는 아니다. "
