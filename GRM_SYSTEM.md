...
| 최종 수정일 | 2026-06-04 |
| 현재 상태 | 매일 수집/Notion 적재 동작, GitHub Actions 내부 health check P1 구현, P1 글로벌 3종은 기본 off 상태로 라이브 검증 대기. **제품군 확장(Phase 4) 코드·프롬프트 완료(`feature/multi-modality` 브랜치) — Notion `Modality` Select 속성 생성 완료(2026-06-04)·preflight 통과. `ENABLE_MODALITY_TAG` 기본 off 유지, 라이브 활성·머지 대기.** Routine 프롬프트는 v15.8 로 개선(다음 라이브 Routine에서 검증 예정) |
...
| `ENABLE_MODALITY_TAG` (제품군 태그 기록) | `false` (워크플로우 wiring·Notion `Modality` 속성 생성 완료 — 첫 라이브 분포 검증 전까지 false 유지) | `false` |
...
| P4-검증 | 워크플로우 wiring 완료(`grm-intake.yml` env·dispatch input). **Notion Intake DB `Modality` Select 속성 생성 완료(2026-06-04, 옵션 Chemical/Biologic/Other) + preflight 통과 검증.** 남은 것: `ENABLE_MODALITY_TAG=true` → 1~2주 라이브에서 화학합성/생물 카드 분류·제품군별 발행 그룹핑·거짓음성(무균오염·CCIT 등 누락) 점검 후 `feature/multi-modality` 머지 | 🟡 Notion속성 생성·preflight 검증 완료 / 라이브 활성·머지 대기 |
