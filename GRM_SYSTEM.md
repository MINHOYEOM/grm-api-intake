...
| 문서 메타 | 값 |
|---|---|
| 문서 버전 | `v1.11` (Phase 4 라이브 재검증 통과 — 한국어 제형 분류 갭 수정 후 81 테스트 green, `ENABLE_MODALITY_TAG=true` 운영 활성·`main` 머지) |
| 최종 수정일 | 2026-06-04 |
| 현재 상태 | 매일 수집/Notion 적재 동작, GitHub Actions 내부 health check P1 구현, P1 글로벌 3종은 기본 off 상태로 라이브 검증 대기. **제품군 확장(Phase 4) 라이브 활성 시작 — a08fe82(한국어 제형 분류 갭 수정) + 25건 실데이터 회귀 추가로 합격기준(정/주→Chemical·한약→Other·생물원료→Biologic·일반어 오탐 0) 모두 통과. `ENABLE_MODALITY_TAG=true` 전환, `feature/multi-modality` → `main` 머지.** Routine 프롬프트는 v15.8 로 개선(다음 라이브 Routine에서 검증 예정) |
| Active phase | Phase 4(제품군 확장) 운영 활성 — 1~2주 발행 브리프 관찰 + Phase 3(P1) 글로벌 확장 검증 + Routine 프롬프트 v15.8 검증 |
| 주요 enabled flags | 운영 기본: `ENABLE_MFDS/RECALL/ADMIN/GMP_INSPECTION=true`, `ENABLE_MODALITY_TAG=true`(2026-06-04 활성), `ENABLE_ICH/WHO/HC=false`, `ENABLE_SEARCH/SCRAPE/MOLEG_API=false` |
