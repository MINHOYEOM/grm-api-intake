...
## 16. deterministic_detail — 결정론 상세보기 슬롯 (MFDS GMP실사, 2026-07-02)
...
결정론 층만 갖는다(분석층 없음). 향후 483 등 다른 결정론 detail 은 `type` 분기로 확장한다.
**[소스확장 2026-07-02]** Federal Register 상세보기가 이 슬롯의 두 번째 `type`(`fr_summary`) 으로
합류했다 — `_deterministic_detail(kind, row, raw)` 의 `kind=="guidance"` 분기(abstract 전문+문서유형).
자세한 FR 슬롯 정의는 §17(B).

**(B) 적용 범위·유형 분기.** `kind == "gmp-inspection"` + 수집기가 `raw.gmp_deficiencies`(구조 배열,
...
**(C) 스키마.** `to_web_card()` 출력의 `deterministic_detail`(`gmp_deficiencies` 존재 시만; 없으면
키 자체 부재 → 요약카드 유지·기존 golden 바이트 불변):
```json
{
...
**(B) Federal Register — 상세보기(결정론만, 분석층 없음) → §16 `deterministic_detail` 로 통합.**
FR abstract 는 FDA 공식 요약이라 충분히 풍부 → **결정론 조립만**(LLM/게이트/외부 fetch 0). §16 의
`deterministic_detail` 이 GMP 지적 표와 **동일 슬롯·`type` 분기**로 설계됐으므로(별도 `detail` 키 신설
대신) FR 은 `_deterministic_detail(kind, row, raw)` 의 `kind=="guidance"` 분기가 **`{type:"fr_summary",
summary_full(abstract 전문·무절단 — quote 는 250자 절단본이라 긴 abstract 는 상세가 값),
detail_kind(Rule/Proposed Rule/Notice/Guidance)}`** 를 낸다(웹 전용 — Notion 마크다운 스캐폴드 불변).
abstract 부재면 미출현(graceful). 적용범위·기업대응(결정론 불가)·comment_close(facts 의견기한 중복)은
생략(창작·중복 방지). `card.html`: §16 `deterministic_detail` 블록의 `type=='fr_summary'` 분기(`.block detail`
재사용, 신뢰 라벨 "원문 기반"). `_fr_detail_kind` 만 유지(구 `_fr_detail`·별도 `detail` 키는 통합으로 제거).
...
계열(WHO PDF)만 의도적 재동결, 웹 full-page 골든 불변. **1075 green**. 비용모델(신규 API 과금 0)·
...
| 2026-07-02 | **§17 신설 — 소스별 상세보기 확장 + 요약카드 보강**: §15 WL 자산을 확장. Phase A(MFDS 행정처분 상세보기+분석층 — `disposition_basis` ②섹션·한국법령 D2·`resolve_required_sections`·`ENABLE_MFDS_ADMIN_BODY_FULL`·한글 섹션명)·Phase B(Federal Register 결정론 상세보기 — abstract 전문, §16 `deterministic_detail` type=`fr_summary` 로 통합·웹 전용)·Phase C(검사·실사 결과 PDF 링크 승격 — `_dual_links` who-inspection→pdf_url·gmp download_url, 신규 블록 없음)·Phase D(UI 보강 — 미리보기 태그·신뢰 라벨·해석 뱃지). 기본 OFF/샘플브리프 부재 → 커밋 골든 additive(guidance_fr.webcard·brief_web·who_inspection 계열만). 1075 green. 6슬롯 §0~§14·§15 WL 경로 불변. |
