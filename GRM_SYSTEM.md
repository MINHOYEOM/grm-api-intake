...
| P4 | **제품군 확장**: 수집기 `compute_modality`(화학합성·생물·기타 3분류)·무균/바이오 키워드·Tier 확장·Notion `Modality` 속성(게이트), 프롬프트 v15.8(제품군 역할·제외규칙·Recall Tier·제품군별 발행). Codex 점검 2회 + 자체 점검 반영(Tier3 floor·Unrelated 가드·Drugs→Chemical·HC 정규화·-mab 오탐 수정·프롬프트 정합). 전체 34 회귀 테스트 green | ✅ 코드·프롬프트 완료 |
