#!/usr/bin/env python3
"""MHRA GMP NCR 과거 findings 딥 백필 — Notion 우회, Supabase 직행.

배경: 매일 크론(`grm-intake.yml`)은 enforcement 윈도우가 `max(window_days, 30)` 이고
`window_days` 상한이 [1,90] 이라 최근 90일만 본다. MHRA GMP 비준수 성명서(SoNC)는
2019년부터 누적된 성긴 소스라, 90일보다 오래된 과거분은 그 경로로는 절대 안 들어온다
(MHRA 필터 자체는 전량을 반환하지만, 수집기가 발행일 창으로 클라이언트측 필터하기
때문). 이 스크립트는 EU NCR 백필(collect_eu_ncr_backfill.py)과 **동일 패턴**으로 넓은
발행일 창을 한 번에 수집해 raw_signals + findings 를 Supabase 에 직접 적재한다(Notion
무접촉 → 주간 브리프 파이프라인 무간섭 — 과거분은 7일 창 밖이라 News 카드로도 안 나간다).

재사용 배관(신규 배관 없음):
  - `collect_mhra_gmp_ncr.collect_mhra_gmp_ncr(start, end)` — 수집 + IntakeItem 생성을
    전부 수행(라이브 라인과 동일 함수). EU 와 달리 PDF/Storage 아카이브 계층 없음(상세
    페이지 URL 이 영속 official).
...
멱등성: raw_signal_id / finding_id 는 (source, document_id=report_no) 해시라, 이미
적재된 건을 다시 적재해도 중복 0. 광범위 창으로 여러 번 재실행해도 안전.

실패는 침묵 금지: 수집 자체 실패(collect_mhra_gmp_ncr error)는 exit 2. 건별 append 실패는
...
SCHEMA_VERSION = "grm-mhra-ncr-backfill/v1"
...
# 이 스크립트가 매개하는 collect_mhra_gmp_ncr / append 는 테스트에서 모듈 이름으로 패치된다.
...
class BackfillReport:
    ...
    would_append: list[str] = field(default_factory=list)  # dry-run: 앞 20건 report_no
...
def run(
    *,
    ...
    # collect_mhra_ncr_backfill.collect_mhra_gmp_ncr / ...append... 를 패치하면 그대로 반영된다.
    collector = collector or collect_mhra_gmp_ncr
    ...
    if err:
        ...
        log("ERROR", f"MHRA NCR 백필 수집 실패: {err}")
        return report, 2
    report.collected = len(items)
    log("INFO", f"MHRA NCR 백필 수집 {len(items)}건 (창 {start}~{end})")
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="MHRA GMP NCR 과거 findings 딥 백필 — Supabase 직행"
        "(Notion 미접촉·collect_eu_ncr_backfill 패턴). 넓은 발행일 창을 1회 수집·멱등 적재.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.dry_run and not (base and key):
        print("collect_mhra_ncr_backfill: --supabase-url/--service-role-key 또는 "
    ...
    try:
        ...
        print(f"collect_mhra_ncr_backfill: 날짜 파싱 실패: {e}", file=sys.stderr)
        return 2
