...
_CITATION_PATTERNS = (
    ...
    #   [반각 코너 브래킷 2026-08-03] 식약처 원문은 전각 「」(U+300C/300D)와 **반각**
    #   ｢｣(U+FF62/FF63)를 문서마다 섞어 쓴다. 반각을 빼먹으면 `｢약사법｣ 제56조…`가 법령명째로
    #   추출·정규화되지 않아, 원문에 조항이 **실재하는데도** D2 가 날조로 FAIL 한다(2026-08-03
    #   실측: admin-2026005914 가 이 한 글자 차이로 오탐 FAIL 3건). 같은 표기의 쌍둥이 코드포인트는
    #   반드시 함께 다룬다.
    re.compile(
...
def _normalize_citation(tok: str) -> str:
    ...
    반각 코너 브래킷 ｢｣(U+FF62/FF63)도 전각 「」와 **같은 표기**다 — 식약처 원문이 문서마다
    섞어 쓰므로 둘을 함께 흡수하지 않으면 실재 조항이 날조로 오탐된다(2026-08-03 실측)."""
