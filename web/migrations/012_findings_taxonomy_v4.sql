-- FIND-1 taxonomy v4 이벤트 -- 이미 살아있는 public.findings 의 taxonomy_version
-- CHECK((v1, v2, v3) IN-list, 011_findings_taxonomy_v3.sql 참조)를 (v1, v2, v3, v4)
-- IN-list 로 확장한다. v1/v2/v3 로 이미 저장된 행은 그대로 보존한다(provenance) -- 이
-- 마이그레이션은 기존 행을 재분류하지 않는다(재분류는 별도 findings_reclassify_service.py 가
-- 담당, grm-findings-reclassify.yml workflow_dispatch 로 재사용 -- 신규 워크플로 불요).
-- 전제: 002_findings.sql(fresh-install 정본) + 004_findings_taxonomy_v2.sql +
-- 011_findings_taxonomy_v3.sql 이 먼저 적용되어 있어야 한다.
...
--
-- ★004/011 의 루프변수-별칭 충돌 함정 재확인: plpgsql record 변수 이름이 FOR 루프 쿼리
...
begin
  ...
  alter table public.findings
    add constraint findings_taxonomy_version_v1v2v3v4_check
...
-- 검증: 현재 저장된 taxonomy_version 값 분포(v1/v2/v3/v4 외 값이 있으면 안 된다).
-- select distinct taxonomy_version from public.findings order by taxonomy_version;
-- 검증: 신규 제약이 존재하고 v1/v2/v3/v4 를 모두 허용하는지 확인.
-- select conname, pg_get_constraintdef(oid) from pg_constraint where conname = 'findings_taxonomy_version_v1v2v3v4_check';
