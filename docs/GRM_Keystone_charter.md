...
- **K2 진행 (🟡 2026-06-05, 별도 Claude Code 세션).** 단계 게이트로 수행(각 단계 끝 테스트 green + 사람 GO/HOLD).
  - **Stage A(M0 부서게이트) + Gate A P1 보정: ✅ origin/main 머지 완료.** 식품 WL(OII 단독 cgmp 관통 포함) 입구 차단.
  - **Stage B(K2-prep raw fetch·하이브리드)·C(`build_card_scaffold`+golden)·D(handoff v2 `ENABLE_HANDOFF_V2` off): ✅ 코드 완료(`feature/keystone-k2`, 133 green) → Codex 재검토 후 일괄 머지 대기.**
  - scaffold 저장위치 = handoff JSON 본문 포함(additive v2). 제목은 §13.1-1·8 동결안(prefix·소재국·DocID 제거).
  - **이월(K3/K4):** inmemory_raw 의 `main()` 와이어링·v2 운영 전환(K3)·Status 실제 전이(K4).
