...
L11 출처 링크 근거(provenance, HARD FAIL — 2026-06-15 W24 회귀): 모든 카드 📰/📎 링크가
    ...
    **MFDS/nedrug 링크**가 본문에 있으면 FAIL — 특히 `mfds.go.kr/brd/*/view.do?seq=`(보도자료
    ...
    패턴 유추(환각) 의심.
L12 scaffold 고정 셀 전사(provenance, HARD FAIL — 2026-06-22 사고): 각 렌더 카드의 W2 표 고정 셀
    (슬롯 아닌 모든 셀 — 발행일·문서번호·FEI·시설유형·Class·실사일·업체 등)이 handoff `card_scaffold`
    값과 **글자 그대로** 일치하는가. FEI 숫자 변경·발행일 일괄 덮어쓰기·시설유형 추론 단정
    (예: `Outsourcing Facility`→`인도 소재 고형제`)·회수등급(`Type III`) 삭제는 모두 FAIL.
    **결정론 게이트 1차**(`brief_lint.lint_scaffold_fixed_cells`=PL18, 항상 실행)이며, 본 항목은 그
    게이트 결과를 인용한다(수기 재대조 금지 — EVAL 만 잡고 Lint 가 놓치던 비대칭 제거).
