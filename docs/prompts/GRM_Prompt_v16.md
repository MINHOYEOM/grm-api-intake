...
Recall·EMA·PIC/S)은 상시 수집기(기본 7종, flag 무관)가 이미 handoff 로 넘기므로, 정상 운영에선
"Intake 흡수로 대체" 로 적고 자유 검색·출처 URL 작성을 하지 않는다 — 다만 수집기 무음 실패(403·
포맷 변동·Intake 게이트 누락)를 잡는 가벼운 cross-check 1회만 허용하고 결과 URL·내용은 지어내지
않는다(handoff 가 정본). **검색이 유일경로라 생략 금지: 슬롯 2(FDA Guidance·수집기 기본 비활성)·
...
는 누락 보강용 보조 cross-check 다. Routine 환경에선 5개 모두 상시 403 이 정상(PL-8) — 403 이어도
출처 URL·내용을 지어내지 말고 Intake 로 충당한다(handoff 에 있으면 그게 정본):
...
정본이라 403 도 정상(비정상 경보·URL 날조 금지), 4·5 도 403 허용.** 5개 전부 실패해도 Routine 은
계속(Intake 가 정본).
