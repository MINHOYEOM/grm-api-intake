...
2. 사실 생성 금지(양방향): 산문 슬롯은 그 카드의 `prose_input`(+해당 시 검색 보강 사실)과 코드가
   ...
   않는다 — 그 자리는 "확인 불가"로 둔다. **반대로, facts/prose_input 에 값이 있으면 반드시 인용**
   한다 — 있는 확정값을 "생성 금지" 명목으로 누락·축소하지 않는다(Lancora 회수등급 `Type III`
   과억제 삭제 ↔ Intas/Dabur 소재·제형 과생성, 양쪽 동시 차단).
2-1. **원문의 부재를 주장하지 마라(2026-07-20 사고 — 최우선 규칙).** "세부 위반내용은 원문에
   명시되지 않았다" · "구체적 관찰사항은 원문에 기재되어 있지 않다" · "세부 위반사항: 원문 미기재"
   같은 서술을 **어떤 슬롯에서도 쓰지 않는다.** 이유: 너에게 가는 `prose_input.body_excerpt` 는
   **300자로 잘린 발췌**라 원문에 무엇이 더 있는지 판별할 근거가 되지 못한다. 실제로 원문 전문
   (2만자·조항별 위반 3~5건)을 확보한 카드가 이 문장 때문에 거짓으로 발행됐다(WL 2건·행정처분
   1건·GMP실사 1건).
   - `prose_input.source_body_captured == true` 면 **원문 본문을 확보한 것이다.** 부재를 말하는
     서술은 그 자체로 거짓이다. 받은 발췌 범위 안의 사실만 쓰고, 상세는 카드의 상세 블록이 따로
     보여준다(네가 요약할 필요 없다).
   - `false` 여도 "원문에 없다"가 아니라 **우리가 못 받았다**고 쓴다 — 둘은 다른 사실이다.
     이때 `prose_input.source_body_absent_reason` 에 사유가 실려 있으면 **그 사유를 그대로**
     쓴다: "원문 PDF 에 텍스트층이 없어(스캔본) 상세를 확인하지 못했다" 처럼. 사유가 비어 있으면
     "원문 확인이 필요하다"로 쓴다. **사유를 지어내지 마라** — 코드가 "스캔·비공개로 상세가
     제공되지 않아"라고 단정했다가 거짓으로 판명된 전례가 있다.
   - **예외는 없다.** 한때 "코드가 채우는 `facts` 칸의 '원문 미기재'는 그 필드가 원문에 없다는
     정직한 표기"라는 예외를 뒀으나 **반증됐다** — Health Canada 회수 6건이 `업체: 원문 미기재`
     로 발행됐는데 원문에는 업체명이 있었다(수집기가 그 칸을 읽지 않았을 뿐이다). 코드도
     원문을 필드 단위로 확인하지 않는다. `facts` 의 값 부재 표기는 이제 "미확인"이며, 그것은
     우리 상태만 말한다.
   - 강제선: 조립 단계 `assemble_publish_brief.lint_false_absence_claims` 가 위반 시 **발행을
     차단**한다.
...
**[다국어 5단계 2026-09-04] 영문 갈래 `en` · `tldr_en`** — 웹의 `/en/` 판이 쓰는 슬롯이다.
· **번역이 아니라 같은 요약의 영어 출력**이다. 원문(대부분 영어)을 요약·정리한 결과를 한국어로
  낼 때와 **같은 사실**을 영어로 쓴다. 한국어 문장을 옮기는 것이 아니라, 원문에서 바로 쓴다.
· 규칙은 한국어 슬롯과 **완전히 동일**하다 — 사실/숫자/URL/업체명/문서번호/날짜/기관 **신규 생성
...
· 표·인용·링크·배지는 손대지 않는다(코드 verbatim — 두 언어판이 같은 값을 공유한다).
...
⚠️ 슬롯 값 안에 `<callout>`·`>`·`###`·`{{토큰}}`·`<table>`·`[!NOTE]` 같은 마크업이나 글머리(`-`)·
   굵게(`**`)를 넣지 않는다(있으면 렌더러가 평문으로 노출). 없는 사실은 "확인 불가"(**"원문
   미기재" 금지 — 규칙 2-1**), KO 원문
...
· `title_issue` — 제목 끝 핵심이슈 1구절(≤25자, 명사형 요약 · 평문). prose_input 의 처분문·사유문
  ...
  이미 보유하므로 회사 유무("업체 미상"·"미확인" 등)를 `title_issue` 에 쓰지 않는다(제목에 회사명이
...
· `key_facts[]` — 핵심 사실 평문 문자열 3개(최대 4). 각 항목은 "라벨: 사실"(예: "사유: 함량부적합
  ...
  블록)에 확정값(빈값·"미확인" 제외)이 있는 필드는 key_facts·implication 에서 "미확인"으로 적지
  않는다 — 그 확정값을 사실로 인용한다. "미확인"은 `facts`·prose_input 양쪽 모두 빈값일 때만
  쓴다(그때도 원문의 부재를 말하지 않는다 — 규칙 2-1). 단, 회수 등급은 아래
  등급 가드가 우선한다.
...
· 회수 등급(Class/Type I·II·III·"등급")은 prose_input 등급 필드(raw recall_class 등) 또는 코드 `facts`
  ...
  또는 "회수 등급: 미확인"(**"원문 미기재" 금지 — 규칙 2-1**).
· 한 슬롯에서 "미확인"으로 적은 항목을 다른 슬롯(title_issue·summary·key_facts)에서
  구체값으로 단정 금지(슬롯 간 모순 금지).
· (역방향) 코드 `facts`/prose_input(w2_facts 등)에 확정값이 있는 항목을 key_facts·implication 에서
  "미확인"으로 적지 않는다 — 미확인↔구체값 모순은 양방향 모두 금지. `facts` 는 결정론 코드
...
· (성분·marker·수치) 성분·marker·수치는 prose_input 원문 표기를 그대로 인용한다. 원문이 'A'(예:
  ...
  4.1%↑·1.5%)가 있으면 "세부 수치 미확인"으로 적지 않고 그 수치를 인용한다.
...
6. 회수 등급: prose_input·코드 `facts` 의 등급 필드가 빈 카드에 key_facts·implication 이 Class/Type
   등급을 단정한 곳 0(D2).
7. 슬롯 간 모순 0: 한 카드 안 "미확인" 항목을 다른 슬롯이 구체값으로 단정한 곳 0(D2).
8. tldr ↔ 카드 1:1: tldr 인용 사건마다 대응 카드가 있다(D6).
9. facts 우선 인용(D8): key_facts·implication 의 "미확인" 각 항목에 대해, 같은 카드의
...
행정처분·FDA 483) **또는** `ncr_translation_ready=true` 카드(GMP 비준수·WHO 실사 — 아래 ④)가
...
수행(위 6슬롯 델타를 만든 뒤, [산출물 예치] 전에):
  ...
  ② 카드 유형별로 아래 **4섹션 JSON** 을 생성한다(유형 판별: `document_id` 가 `fda483-…`=483 ·
     ...
     · WL       : `{key_violations:[{citation,original,description,risk}], fda_evaluation, required_remediation:{deadline,items[]}, administrative_risks}` (+ 카드별 `violations_ko` ★필수 — 4섹션 밖·아래 ⑤)
  ...
  ⑤ **[WL 위반항목 국문 병기]** `wl_violation_translation_ready=true` 카드(FDA Warning Letter —
     ...
     **각각을 번역만** 하고(새 사실·해석 금지), ③의 deep 델타에서 **그 카드의 항목 안에**
     `violations_ko` 로 담는다 — `deep_analysis`(4섹션) **안에 중첩 금지**, 나란히 둔다:
       `{"<web_card_id>": {"deep_analysis": {...4섹션...}, "violations_ko": [{"number": "1", "statement_ko": "..."}, ...]}}`
     `number` 는 입력값 그대로 보존한다(병합이 **번호로 1:1 매칭** — 순서가 아니다). 조항 인용은
     원문 표기 그대로 문장 끝 괄호로(예: "…절차를 수립하지 않았다 (21 CFR 211.100(a)).").
     확인(identity)·함량(strength)·품질(quality)·순도(purity) 등 핵심 GMP 용어는 국문(영문) 병기.
     ⛔ **`violations_ko` 는 필수다 — 빠지면 그 주 브리프 "전체"가 발행되지 않는다**(2026-08-24
     신설: 병합층 #670 이후에도 이 지시가 없어 08-24 발행분 WL 5카드·14표제문이 영문 단독으로
     나갔다). 483 `observations_ko` 와 동일한 2겹 브리프 단위 차단이다:
       · `web/render.py: validate_wl_violations` — 배포 빌드 fail-closed. 어느 위반이든
         `statement_ko` 누락이면 **사이트 빌드 전체가 FAIL**.
       · `assemble_publish_brief: _lint_wl_violation_ko`(게이트 6) — 조립 단계에서 같은 규약 선행 검출.
     심층분석 4섹션이 게이트에서 FAIL 해 drop 되더라도 `violations_ko` 는 별도 층으로 살아
     병합된다 — 분석이 자신 없는 카드라도 위반 표제 번역만은 반드시 채운다.
...
파이프로 넘긴다(네가 handoff 를 Notion 에서 읽어오는 것과 정확히 대칭 — 이번엔 네가 쓰는 쪽).
  - **새 페이지 1개** 생성 — 반드시 **Intake DB 의 행(row)** 으로 만든다. 페이지의 부모(parent)를
    이 DB 의 **data source**(`d5b9634a-2bd7-4036-ba06-e4ad17ede288`, DB `7784c71fb7b343749b2bee5d04db7926`)
    로 지정해야 한다. ⚠️ 부모를 지정하지 않으면 Notion 은 **워크스페이스 최상위 독립 페이지**로 만들어
    버리는데, 그런 페이지는 DB 행이 아니어서 아래 속성을 가질 수 없고 → 브릿지의 DB 쿼리에 **안 잡혀
    조용히 유실**된다(2026-07-12 실장애 원인). 제목 `OPEN GRM Web Delta {publish_date}`
    (예: `OPEN GRM Web Delta 2026-07-13`, `publish_date` = 이 handoff 의 `run_date_kst`).
  - 속성(**필수** — 옵션은 이미 스키마에 등록돼 있다): `Source` = `GRM Web Delta` ·
    `Type or Class` = `web-delta` · `Status` = `New`. 이 세 속성은 브릿지의 선택 필터
    (`Type or Class`=web-delta ∧ `Status`=New)와 정확히 일치해야 하며, 하나라도 비면 유실된다.
  - **예치 후 자기검증(필수)**: 예치 직후 이 Intake DB 를 `Type or Class`=`web-delta` ∧
    `Status`=`New` 로 다시 쿼리해 **방금 만든 페이지가 결과에 나오는지 확인**한다. 나오지 않으면
    독립 페이지로 잘못 만들어진 것 — 그 페이지를 이 data source 로 **이동(move)** 시키고 위 세 속성을
    설정한 뒤 다시 쿼리해 확인한다. **DB 쿼리에 잡히는 것을 확인하기 전에는 이 단계를 끝내지 않는다.**
