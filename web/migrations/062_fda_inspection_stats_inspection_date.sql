-- ============================================================================
-- 062_fda_inspection_stats_inspection_date.sql
...
-- ★이 파일이 적용된 뒤에는 **프로덕션 현행 정의의 정본이 062 이다**(059 는 원복용
...
as $$
  with base as (
    select inspection_id, classification_code, fiscal_year, country_key, country_name,
           ...
           -- [062] posted_citations — 058 이 적재해 두고 어디서도 쓰지 않던 축.
  ...
  select jsonb_build_object(
    ...
    -- ══ [063 신설 ①] by_quarter — **실사 종료일** 기준 분기 집계 ═══════════════
    ...
    -- ══ [063 신설 ②] korea — 한국 소재 제조소 연도별 슬라이스 ═══════════════════
