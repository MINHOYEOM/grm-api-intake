...
def document_view(doc: dict[str, Any], *, min_findings: int,
    ...
    for f in doc.get("findings") or []:
        ...
        # [다국어 4단계 2026-09-04] 규제기관이 쓴 **원문**. 영어판 문서 페이지의 본문이
        # 이것이다 — 공개 지적의 91.6%가 원문 영어라, 새로 번역하는 게 아니라 덧씌운
        # 한국어 층을 걷어내면 된다(설계 문서 §1).
        #   ★국문과 **다를 때만** 싣는다. 원문이 한국어인 지적(식약처 등 8.4%)은 두 값이
        #     같아서, 넣으면 같은 문장을 파일에 두 번 적는 것이다. `cfr_refs`·
        #     `inspector_names` 와 같은 "값이 있을 때만 키를 싣는다" 규율의 연장이다.
        #   ★위 "혹시 쓸까 봐 금지" 규율과 어긋나지 않는 근거: 소비처가 확정됐다
        #     (영어 문서·모음·조항 페이지). 소비처 없이 미리 넣지 않는다는 뜻이지,
        #     쓰는 값을 빼 두라는 뜻이 아니다.
        orig = (f.get("finding_text") or "").strip()
