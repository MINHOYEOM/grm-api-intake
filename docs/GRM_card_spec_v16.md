...
3. **W2 = 사실 표(경량)**: 발행일 · 문서번호(`MARCS`·`admin-…`·`FR …`) · 유형별 핵심 행 = 4행. 라벨 이모지 없음. Evidence/Signal/원문언어/제품군 행 없음(중복). 첫 행 라벨은 `SourceSpec.date_label`(기본 `발행일`)이며 **원천이 게시일을 주지 않는 소스만** 바꾼다 — `who-inspection`(WHOPIR)은 목록이 실사일만 주므로 `실사일`(2026-08-10). 값이 없으면 `card_scaffold.VALUE_UNKNOWN`("미확인").
