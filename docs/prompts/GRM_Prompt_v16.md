...
1. scaffold 불변: handoff v2 의 `card_scaffold` markdown 은 슬롯 토큰({{...}}) 치환 외에
   ...
   **또한 W2 표의 identity 고정 셀(FEI 동반 제조소/업체·문서번호·시설유형·Class·제품·업체)은
   전 소스 글자 그대로 전사한다. 재생성·추론·삭제·단정 보강 금지** — 예: FEI 숫자 변경(`FEI 1073935`
   →다른 번호), 시설유형을 `인도 소재 고형제`처럼 추론 단정, `Outsourcing Facility`→`Manufacturer`
   바꾸기, 회수등급 `Type III` 삭제. 값이 비어 있으면(`원문 미기재`) 비운 채로 둔다.
   **날짜 셀은 소스별로 다르다**: `FDA 483`·`MFDS 행정처분`의 발행일·처분일·실사일은 수집기가 원문에서
   추출한 정본이니 그대로 두고 여러 카드를 단일 날짜로 덮지 않는다(06-22 FDA 483 발행일·admin 처분일
   오류 클래스). 반면 `FDA WL`·`FR`·`ECA`·`HC`의 발행일은 **수집일 placeholder** 라 WebSearch 로
   실제 문서일자(예 WL 03-19·ECA 기사일)로 보강(enrich)하는 것이 정상이다.
   실제 강제선은 `brief_lint.lint_scaffold_fixed_cells`(PL18 — identity 셀 전 소스 + 483/admin 날짜)·
   발행 후 `verify_published_brief` 이며, 본 문구는 MCP 전용 세션의 보조 가드다.
