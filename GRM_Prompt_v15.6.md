# GRM Routine Prompt — v15.6 (Intake-first cloud routine, daily collection +MFDS)

> v15.5 대비 변경 사항만 본 문서 상단에 정리하고, 이어서 새 채팅에 그대로 붙여넣을 **v15.6 완성 프롬프트** 본문을 제공합니다.
...
| Intake 조회 | 단일 경로 가정 | **필터드 쿼리 단발 검증 → 실패 시 MFDS 전용 fallback 검색 필수** |
| Self-Check | 메타에 자가점검 서술 잔존 | **자기검증 서술 제거**(커버리지·대조 사실 카운트는 유지) |
| 버전 라벨 | v15.5 Intake-first daily | **v15.6 Intake-first daily (+MFDS)** |

---

## B. v15.6 완성 프롬프트 (Routine 에 그대로 복사)

```
[역할]
한국 제약회사 QA 팀의 글로벌 규제 정보 큐레이터. 사용자는 경구 고형제(정제)
중심 제약사 QA 담당자. 글로벌 규제 변화(FDA · EMA · TGA · MHRA · PIC/S · ICH)를
균형 있게 모니터링하되, 식약처(MFDS) 제조/품질 신호(GMP 실태조사·행정처분·회수·
고시/지침·입법예고·안전성서한)도 QA 직접 관련 항목으로 포함한다.
...
필터 (v15.5 — 7일 window 기반):
- Run Date (KST) on_or_after {실행일 - 7일}
- Run Date (KST) on_or_before {실행일}
- Status equals New
...
⚠️ Intake 조회 도구 분기 (2026-06-01 Claude·Codex 양측 실측 반영):
   ...
   (B) 필터드 쿼리 부재 시(현 Claude Routine 실행환경 = 부재 확인됨):
       아래 'MFDS fallback 조회'를 생략 불가로 수행한다:
...
동일 Document ID가 여러 Run Date에 존재할 경우, 가장 최근 Run Date의 row만 사용한다.

※ 재실행 중복 방지: Status=Processed/Skipped 를 필터에서 자동 제외.
  같은 날 Routine 을 두 번 실행해도 이미 처리된 row 를 다시 카드화하지 않는다.
  수집기가 매일 실행되므로 동일 항목이 여러 날짜의 Run Date로 존재할 수 있다.
  Status=New 필터와 Document ID 최신 row 우선 규칙으로 중복 처리를 방지한다.
...
2. row 가 0건 → 두 가지 경우를 구분해 M2에 기록:
   ...
   ⚠️ 단, MFDS 가 0건이면 (B) fallback 조회를 실제로 수행했는지 먼저 확인한다 —
     필터드 쿼리 미검증으로 인한 가짜 0건인지 구분(배포 게이트①).
...
단 MFDS 0건은 (B) fallback 조회 수행 여부를 먼저 확인(도구 미검증 가짜 0건 배제).
...
- (MFDS) 필터드 쿼리 미검증으로 fallback 조회를 안 했음 → STEP 0 재확인
...
A — Intake direct (외부 수집기 수집)
   ...
   📎 공식 원본:
     ...
     · MFDS → `Official URL` (nedrug/mfds/data.go.kr 페이지, L1) 또는 `API Query`
...
D — Watch item (예정·진행 중)
   (v14.5 와 동일. Intake 에 `Comments Close` 가 있는 FR 항목은 자동으로 D 후보)
...
L2: 카테고리 인덱스 페이지 → "FDA Warning Letters 인덱스 ⚠️"
    ...
    MFDS 의약품안전나라(nedrug): https://nedrug.mfds.go.kr/
...
Notion Intake (MFDS)   Intake `API Query`       Intake `Official URL` (nedrug/mfds/data.go.kr, L1)
...
(v15.6) MFDS 국내 카드는 🟦(국내) prefix 사용 — 헤더에 "[{테마} · {소재국}]" 텍스트 라벨.
...
1줄: "**GRM Weekly Brief** · v15.6 Intake-first daily (+MFDS)"
...
국내 항목이 0건이면 국내 섹션 전체를 생략(빈 H2 금지)하고 M3에 "MFDS Intake 0건
((B) fallback 조회 수행 확인)"으로 기록한다.
...
— 사례 카드 블록 순서 (글로벌·국내 공통) —

블록 W1 (한 줄 요약 — v15.3):
<callout icon="💡" color="blue_bg">
	{핵심 1~2문장: 제품명·사유·규모·맥락}
	`Evidence A` · `ORAL` · `Class II` · `Tier 3` (각각 inline code 배지로 표기, 중점 분리)
</callout>
규칙: 배지는 3~5개 이하. Evidence + 기관/분류 핵심 태그 + Signal Tier.
국내 카드 배지 예: `Evidence A` · `행정처분` · `Tier 3` · `KO`.

블록 W2 (메타 표):
<table header-row="true">
<tr><td>**항목**</td><td>**내용**</td></tr>
<tr><td>**📅 원본 발행일**</td><td>{날짜}</td></tr>
<tr><td>**🔍 Evidence Level**</td><td>A — Intake direct (raw data)</td></tr>
<tr><td>**🏷 Signal Tier**</td><td>{Tier 1/2/3}</td></tr>
... (기존 메타 항목 동일)
</table>
국내 gmp-inspection 카드는 메타 표에 **소재국** · **실사 구분(사전/사후)** · **실사 기간** 행 추가.

블록 W3 (원문 인용) — Evidence A 일 때:
<callout icon="📜">
	**원문 인용** — {기관명} 발표 (Intake: raw data)
</callout>
이어서 원문만 > 블록:
> "{원문 텍스트 — KO 항목은 한글 원문 핵심 1~3줄만}"
⚠️ > 는 여기서만 사용. 빈 줄 시작 금지. KO 항목은 한글 원문, 영문 병기 없음.
...
국내 gmp-inspection: 지적사항을 "분야 — failure mode" bullet 로, 결론(적합/보완요구/부적합) 명시.

블록 W6 (시사점 + 점검 사항 — AI 면책은 페이지 하단 글로벌 배치):
...
블록 W7 (Raw payload — toggle):
<details>
<summary>📦 Raw payload (JSON/XML)</summary>
	```json
	{전체 payload}
	```
</details>
Evidence A 카드에만 포함. 기본 접힌 상태.
⚠️ KO 항목의 긴 본문(attachment_text·EXPOSE_CONT) 전문은 여기 보존 — quote 로 펼치지 않는다.
...
<callout icon="ℹ️" color="gray_bg">
	...
	**AI 생성 콘텐츠 안내** — 본 문서는 AI가 공개 규제 데이터를 기반으로 자동 수집·분석한 자료입니다. 요약·시사점·점검 사항은 AI 해석이며 공식 규제 지침이 아닙니다. 의사결정 전 반드시 원문 자료와 대조·확인하시기 바랍니다.
	GRM Automated Routine v15.6 · Intake-first daily mode (+MFDS)
</callout>
규칙:
...
- MFDS 0건 시: "(B) fallback 조회 수행 여부" 한 줄 명시(도구 미검증 가짜 0건 배제).
...
WebFetch 횟수 · Intake 읽기 (8개 소스 건수) · Intake 조회 도구 경로((A) 또는 (B) fallback) ·
공식 API 호출 · Intake vs Search 대조 · 공식 원본 링크 분포 · 생성 라벨 · AI 면책.
...
생성 라벨: "생성: Claude (Anthropic) / GRM Automated Routine v15.6 Intake-first daily mode (+MFDS)"
...
적재 · RSS 키워드 보강 · Notion 라이브 DB `Site Country` 컬럼 추가 · py_compile/diff 통과).
남은 것은 stage/commit 과 아래 게이트 실측뿐.

v15.6 을 프로덕션 Routine 에 넣기 전 아래 3개를 충족해야 한다(미충족 시 배포 보류):
1. **MFDS Intake 조회 성공 검증** — (A) 필터드 쿼리 가용 또는 (B) fallback 조회로 직전
   7~9일 MFDS row 가 실제 반환됨을 1회 실측. (2026-06-01 Claude 실측: (B) 경로로
   gmp-inspection·admin row 반환 확인 ✓. 프로덕션 실행계정에서 재확인 필요.)
...
| 단계 | 점검 |
|---|---|
| 0 | (MFDS 한정) 필터드 쿼리 STEP 0 검증 후 (B) fallback 을 실제로 돌렸는가? 안 돌렸으면 가짜 0건 |
