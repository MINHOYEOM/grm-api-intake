...
## 5. 성공 기준 — 두-트랙 (2026-06-22 개정)

> 개정 사유: 기존 "4주 연속 Lint 0 (all-or-nothing)" 은 위험·비위험을 안 가렸고, 적대적 채점기가
> **조용한 사실오류**(예: 6/22 출처 링크 날조 5건 중 3건 silent)를 신뢰성 있게 못 잡아 이 제품의
> 위험모델과 안 맞았다. 사실·링크는 **결정론 게이트(증명 가능)**, 해석 품질은 **채점기(스트릭)** 로 분리한다.
> 상세 = 루트 `GRM_로드맵_현황통합_2026-06-22.md` §4.

**상시(전제):** 동일 handoff 입력 → 카드 골격 결정론(golden 통과) · 카드 양식 실행 간 무변형.

**① 안전망 트랙 (사실·링크 = 결정론 게이트) — 커버리지로 측정, 리셋 없음.**
- 카드의 사실 요소 전부(출처 링크·회사명·날짜·회수등급·성분·FEI·위반사유)가 수집기 원본과 글자단위 일치하는지를 코드 게이트가 발행 전/후 대조(URL provenance·PL14 요일·PL15 수집·PL18 셀·PL19 집계·모달리티 GAP-2 등).
- AI 가 사실·링크를 몰래 바꿀 수 있는 경로를 목록화하고 각각 게이트로 막는다. 새 경로 발견 = 점수 차감이 아니라 게이트 추가(영구).
- Status 갱신·handoff CONSUMED 누락 0(주간 재유입 0).
- **완성 판정 = 깊은 적대적 조사로도 새 Sev-1(오도 위험) 경로가 더는 안 나옴 + 게이트 매주 통과.**

**② 품질 트랙 (해석 = 채점기 EVAL-1) — 심각도 스트릭.**
- 원본에 정답이 없는 부분(시사점 유용성·점검 실용성·복붙·제목)만 채점.
- Sev-1(오도 위험) 무관용 0 · Sev-2(과소표기 등) 주당 ≤2·비증가·최종주 ≤1 · Sev-3(외관) 로그만(리셋 안 함).
- 시사점·점검 품질이 v15.8 대비 저하 없음 이상.

**K3 종료 = ① 안전망 완성 + ② 품질 Sev-1 0(4주). 상용화/베타 게이트도 동일선 공유(베타는 6/29 선오픈).**
...
- **K2 완료 (✅ 2026-06-05 origin/main 머지됨, 별도 Claude Code 세션).** 단계 게이트로 수행(각 단계 끝 테스트 green + 사람 GO/HOLD).
  ...
  - **K3 출발 문서: `archive/point-in-time/GRM_Keystone_K3_인계메모.md`** (Codex 종합점검 인계 + prose_input 최종 스키마 + 결정 목록 7건 + 성공기준). 새 채팅에서 "Keystone K3 시작" + 이 문서 지정.
...
| 2026-06-22 | **§5 성공기준 두-트랙 개정**: all-or-nothing "4주 Lint 0" → ① 안전망(사실·링크 결정론 게이트, 커버리지·리셋없음) + ② 품질(EVAL-1 Sev 스트릭). 사유=적대적 채점기가 조용한 사실오류(6/22 URL 날조)를 못 잡아 위험모델 불일치. 상세 루트 `GRM_로드맵_현황통합_2026-06-22.md` |
