...
#### Phase 2c Decision — Self-Check Source Feasibility (2026-05-29)
**Decision**
- GMP 실태조사 결과 소스는 **Phase 2c에서 defer**(제외).
- Phase 2c 첫 구현 대상은 **회수·판매중지(data.go.kr service `15059114`)** → Type `recall-quality`, Tier 3.
- `Self-Check Required` 필드(`Yes`/`Review`/`No`) **추가 완료**. 우선 `safety-letter`·`recall-quality`에 적용.

**Rationale**
- 실태조사 데이터는 자가점검 가치 최상위지만 깨끗한 공개 API/피드 미확인.
- `nedrug.mfds.go.kr`는 JS 렌더링 SPA — 정적 fetch 빈 응답(spike 확인). 정적 스크래핑 불가.
- SPA 백엔드 JSON 엔드포인트를 브라우저 DevTools/XHR로 식별하면 수집 가능(조건부 Medium). 확인 전까지는 Hard로 분류·연기.
- 회수는 data.go.kr API 경로가 깨끗하고 `회수사유내용`이 품질 자가점검 신호로 직접 활용 가능.

**Follow-up**
- 사용자: data.go.kr service `15059114` 활용신청. 신규 시크릿 **`DATA_GO_KR_SERVICE_KEY`**(기존 `grmintake` OC와 분리).
- `recall-quality` collector 먼저 구현(스펙: `Phase2c_recall_collector_spec.md`).
- 실태조사는 별도 브라우저 XHR spike 후 include/abandon 결정 (Phase 2d 후보).

#### 진행 상태
- ✅ Notion `Self-Check Required`(Yes/Review/No), `Type or Class: recall-quality` 추가 완료(라이브 DB, 기존 옵션 전부 보존).
- ✅ `DATA_GO_KR_SERVICE_KEY` 발급·등록 완료(사용자).
- ✅ **회수 API probe 완료**(run 26626859933) — 스키마 확정: End Point `.../getMdcinRtrvlSleStpgelList03`, Decoding+json, item keys(PRDUCT/ENTRPS/RTRVL_RESN/ENFRC_YN/RECALL_COMMAND_DATE/...), dedupe 복합키.
- ✅ **회수 collector 구현·검증 완료**(브랜치 `codex/mfds-recall-quality`): `collect_mfds_recall.py` 신규, `ENABLE_MFDS_RECALL`(기본 false). dry-run 30일 59건/7일 4건, 실삽입 4 → 2회차 skip_dup 4(dedupe 입증), py_compile/diff 통과.
- ✅ **Self-Check v2 적용 완료**(commit 6a12b23): 표시기재·제조번호·유효기간·허가사항과상이 제거 → 표준코드 오기재 샘플 Review 확인. 새 비율 30일 45/59, 7일 3/4. 높은 Yes는 **데이터가 실제 품질결함 회수 위주**라서이며 정당(라벨/사무 오탐은 제거됨).
- ✅ **Claude 최종 판단: ENABLE_MFDS_RECALL=true GO** (오탐 클래스 제거 검증 + 남은 Yes 정당 + 7일 4건으로 부담 사소 + 파이프라인 전구간 검증).
- ✅ v1 잔존 row(레이헥솔주300, 표준코드 오기재) `Self-Check=Yes→Review` 1회 수동 정정(Claude, Notion MCP). (QA Relevance=Likely는 v1 잔존이나 무해 — Routine이 최종 판정.)
- ✅ **`ENABLE_MFDS_RECALL=true` 전환 완료**(commit `ac0f7a5`, main+feature 동일 HEAD). 검증 run 26631747325 green: dry-run 7일 RSS 5 + Recall 4(Self-Check Yes 3/Review 1), error 없음. `ENABLE_MOLEG_API=false` 유지.
- ⏰ 첫 scheduled run 관찰 예약: 2026-05-30 ~05:17 KST 실행 → 06:20 KST 스레드 확인.
- ⏸ GMP 실태조사: defer (브라우저 XHR spike 대기 — Chrome 현재 미연결).

### Phase 2c 결정 변경 — Self-Check 기능 제거 (2026-05-29, 사용자 결정)
**자가점검(Self-Check)은 GRM 수집 파이프라인에서 제거.** 사유: 사용자 최종 비전은 "사용자가 본인 회사 제품을 직접 설정해 관련 규제정보를 매칭받는 별도 웹사이트/시스템"이고, 자가점검은 그 제품의 핵심 기능이다. 수집기가 키워드로 추정하는 현재 방식은 그 미래 기능의 정확도 기대치를 흐리므로 지금은 불필요.

제거 범위:
- ✅ **회수·판매중지(`recall-quality`) 수집 자체는 유지** — Self-Check 없이 일반 규제 모니터링으로.
- 🔧 **Codex 코드 작업**: `collect_mfds_recall.py`(및 관련)에서 `Self-Check Required` 산정/매핑 로직 제거. `build_notion_properties`의 `PROP_SELF_CHECK` 기입 제거. IntakeItem의 self_check 필드는 제거 또는 미사용. 회수 insert는 그대로 동작해야 함. (v2 키워드 튜닝은 moot)
- ❓ **Notion `Self-Check Required` 필드**: 사용자 확인 대기 — (a) 그대로 휴면(무해) 또는 (b) 드롭. 드롭은 파괴적 스키마 변경이라 Claude가 임의로 안 함.
- 자가점검은 **로드맵: 미래 "사용자 설정형 규제 매칭 제품"**으로 이관.

### Phase 2c 남은 항목
1. (제거됨) ~~자가점검 Weekly Brief 액션~~ → 미래 별도 제품으로 이관.
2. GMP 실태조사 nedrug XHR spike (Chrome 연결 시) — 여전히 defer 후보.

### MFDS 제조/품질 커버리지 로드맵 (2026-05-29 — 우선순위 = 제조/품질 정보 빠짐없이 intake)
사용자 우선순위: 식약처 제조/품질 정보(특히 GMP 실태조사 결과) 누락 없이 수집. 조사 결과 소스별 난이도:

| 우선 | 소스 | 경로 | 난이도 | 상태 |
|------|------|------|--------|------|
| 1 | 의약품 행정처분 (`15058457`, `MdcinExaathrService04/getMdcinExaathrList04`) | data.go.kr API | 낮음(recall 패턴) | 미수집 — 다음 후보 |
| 2 | GMP 적합판정서 발급현황 (`15097207`) | data.go.kr API | 낮음 | 미수집 — 다음 후보 |
| 3 | GMP 실태조사(실사) 결과 | nedrug `/pbp/CCBBD03` **서버렌더 HTML**(SPA 아님) | **Medium** — HTML 스크래핑(FDA WL 패턴) | **수집 가능** — 구현 후보 |

**GMP 실태조사 spike 결과 정정(2026-05-29, Chrome 확인):** 이전 "nedrug=SPA라 불가" 판정은 `/index`(SPA 셸) 기준 오판이었음. 실제 결과공개 보드 **`/pbp/CCBBD03`는 서버렌더 HTML**(문서 GET 1회로 표 렌더, XHR 없음). 총 624건.
- 목록 컬럼: 사전/사후, 완제/원료, 국가, 제조소명, 소재지, 실사시작일, 실사종료일, 실사결과[다운로드], 등록일.
- 폼 GET(`/search`), hidden `page/limit/sort/searchYn` → **`?page=N&limit=10` GET 페이지네이션**. 지적사항 본문은 행별 `downFile()` 첨부(PDF/hwp).
- → collector 가능: **목록 메타 스크래핑(v1)** + 다운로드 링크 보존, 지적사항 본문 파싱은 후속. Type `gmp-inspection`(Notion 기존), Tier 2~3. API 키 불필요(공개 웹).
- ✅ **일반 GET 검증(Codex)**: 쿠키 없이 `/pbp/CCBBD03/getList?page=1&limit=10`(및 `limit=100`) 200 + HTML 표. form GET `page/limit/sort/sortOrder/searchYn`. 다운로드 체인 `downFile(docId)`→`/cmn/edms/down/{docId}`(PDF, HWP 섞임 가능).
- ✅ **구현 스펙 작성**: `Phase2d_gmp_inspection_collector_spec.md`. Notion 스키마 변경 불필요(`gmp-inspection` Type·`Official Page Scrape` 기존). API 키 불필요(공개 웹).
- **우선순위: gmp-inspection > gmp-certificate**(자가참조 가치 높음, 블로커 없음). 다음 구현 대상 후보.
- ✅ **gmp-inspection collector 구현·검증 완료**(브랜치 `codex/mfds-gmp-inspection`, commit `2dcae3e`): `collect_mfds_gmp_inspection.py`, `ENABLE_MFDS_GMP_INSPECTION`(기본 false). dry-run 7일 5건, 실삽입 5 → 2회차 skip_dup 5(dedupe 입증). `sort=registTsChar`는 과거 row 함정이라 기본 최신정렬+page/limit만 사용.
- ✅ **Claude 라이브 교차확인**: 5건 모두 진짜 최신 GMP 실사 결과(국내 사후실사). 매핑 정확(Type=gmp-inspection/Source Type=Official Page Scrape/Tier2/Firm=제조소명/Date=등록일/Source URL=다운로드/document_id=gmpinspect-{docId}), Self-Check 미설정. → **ENABLE_MFDS_GMP_INSPECTION=true GO.**
- ✅ **`ENABLE_MFDS_GMP_INSPECTION=true` 전환 완료**(commit `b4f60c0`, main+브랜치). dispatch 기본값 true + scheduled fallback `vars.ENABLE_MFDS_GMP_INSPECTION || 'true'`. heartbeat가 첫 run에서 MFDS RSS/Recall/Admin/GMP Inspection 카운트 + `ENABLE_MOLEG_API=false` 확인하도록 갱신.
- ✅ **Phase 2d-2a 본문 텍스트 추출 구현·검증·main 반영 완료**(commit `4068db3`): PDF(PyMuPDF)+HWPX(stdlib zipfile+ET), 레거시HWP/스캔/실패는 메타+링크+`attachment_parse_status` graceful. 지적 없음→Tier2/Possible, 있음→Tier3/Likely. nedrug 끊김 대응(재시도+1초). 30일 dry-run pdf-ok 22(지적 있음 11/없음 11), 실삽입 22→17 inserted(기존 5 dedupe)→2회차 skip_dup 22.
- ✅ **Claude 라이브 교차확인**: 엘앤씨바이오 row에 지적(보완)사항 표 전문(위탁시험기관 평가·세척 밸리데이션·컴퓨터시스템 밸리데이션·시설관리 등) Body 저장, parse_status=pdf-ok, Tier3/Likely 정확. → **"무슨 지적이었나" 원문 확보 = 2d-2a 목표 달성.** 코드 main 라이브(flag 이미 true) → 다음 scheduled run부터 본문 포함.
- 참고(비차단): 기존 메타-only 5건(가스 제조소)은 dedupe로 body 미갱신 — 필요 시 1회 backfill 선택. 본문은 평문(표가 줄바꿈으로 펼쳐짐) — 구조화는 2d-2b.
- ⏸ gmp-certificate: 2d-2a 후. 403 재-probe부터.
- ✅ **Phase 2d-2 feasibility probe 완료(2026-05-30)**: 다운로드 URL(`/cmn/edms/down/{docId}`) 일반 GET 가능. 샘플 페이지 1/2/5/10/20/40/63 첫 첨부 모두 PDF, PyMuPDF(`fitz`) 텍스트 추출 성공(약 489~1,285자). 최근 5건은 `지적(보완)사항 없음`까지 추출, 과거 샘플에서는 실제 지적사항(품질경영/위탁시험기관 추가 평가자료, 허가관리, 시설장비 등)도 확인. 결론: **현재 v1은 "인덱스 완성", 2d-2a는 PDF 본문 원문 추출로 가치 점프 가능**. HWP/HWPX는 graceful fallback(링크 유지) 후 별도 처리.

- 1·2는 기존 `DATA_GO_KR_SERVICE_KEY` 재사용(각 서비스 data.go.kr 활용신청 1회 추가) + `collect_mfds_recall.py` 동일 구조 → 저비용. Notion Type 신설 후보: `admin-action`, `gmp-certificate`.
- 행정처분(1)은 GMP 위반·품질부적합 제재가 들어와 실태조사 "결과(제재)" 측면을 일부 대체. 실태조사 "지적사항 본문"(3)은 nedrug 경로뿐이라 별도 난제.
- 이미 라이브: GMP 지침/안내서(RSS), 입법예고/고시/개정법령(RSS), 안전성서한(RSS), 회수·판매중지(API).

#### 행정처분 + 적합판정서 진행 상태 (2026-05-29)
- ✅ 사용자 승인: 1·2 즉시 구현, 3(실태조사)은 별도 spike 트랙.
- ✅ Notion `Type or Class`에 `admin-action`, `gmp-certificate` 추가 완료(28개, 기존 보존).
- ✅ 구현 스펙 작성: `Phase2c_admin_gmpcert_collector_spec.md` (probe→구현→검증→enable).
- ✅ **활용신청 승인 완료**(2026-05-29): `15058457` 의약품 행정처분, `15097207` GMP 적합판정서발급현황 (둘 다 개발계정 승인, 만료 2028-05-29). 기존 `DATA_GO_KR_SERVICE_KEY` 재사용.
- ✅ 범용 probe 스크립트 `probe_datago.py` 추가(두 서비스 + 향후 재사용).
- ✅ **행정처분 probe 확정**(probe 브랜치 `codex/mfds-admin-gmpcert-probe`): endpoint `.../MdcinExaathrService04/getMdcinExaathrList04`, Decoding+json, totalCount 624. 필드 `ADM_DISPS_SEQ`(dedupe키)/`ENTP_NAME`/`ITEM_NAME`/`EXPOSE_CONT`/`ADM_DISPS_NAME`/`BEF_APPLY_LAW`/`LAST_SETTLE_DATE`(date) 등. 매핑 확정(스펙 참조). → **admin-action collector 구현 GO.**
- ⏸ **적합판정서 403 보류**: 같은 키로 회수·행정처분은 되는데 `getDrugGmpStbltJgmtIssuStusInq`만 403(후보 404). 진단=서비스↔키 연결 미전파 추정. → 몇 시간 후 재-probe, 지속 시 마이페이지 계정 확인. (op명/필드는 공식문서 기준 잠정 기록)
- ✅ **admin-action collector 구현·검증 완료**(브랜치 `codex/mfds-admin-action`): `collect_mfds_admin_action.py`, `ENABLE_MFDS_ADMIN`(기본 false). dry-run 30일 21건(Tier3=17/Tier2=4, filtered 32)·7일 3건, 실삽입 3 → 2회차 skip_dup 3(dedupe 입증). 저가치 도메인(화장품/광고/마스크) 필터 강화 반영.
- ✅ **Claude 라이브 교차확인**: 3건 모두 진짜 의약품 제조/품질 처분(제조업무정지·기준서/회수절차 미준수). 매핑 정확(Firm/Type/Tier3/Date=LAST_SETTLE_DATE/dedupe=admin-{ADM_DISPS_SEQ}), Self-Check 미설정 확인. → **ENABLE_MFDS_ADMIN=true GO.**
- ✅ **`ENABLE_MFDS_ADMIN=true` 전환 완료**(commit `2d90c5d`, main+브랜치). workflow 기본값 true + scheduled fallback `vars.ENABLE_MFDS_ADMIN || 'true'`. main 수동 dry-run 3건(Tier3=2/Tier2=1) green. 첫 scheduled run 관찰 예약 2026-05-31 05:45 KST.
- 🔧 (비차단 폴리시) Headline: `ITEM_NAME` null 시 `ADM_DISPS_NAME` 전문이 들어가 제목 길어짐 → 추후 첫 줄/요약만 쓰도록 다듬기 가능.
- ⏸ gmp-certificate: 403 지속 시 재-probe 후 동일 흐름.
- 주의: Self-Check 미사용. Signal Tier는 admin-action=Tier3(품질위반)/Tier2(기타), gmp-certificate=Tier1(상태).
