...
| 문서 메타 | 값 |
|---|---|
| 최신 변경 | `v1.49` (**§1-B scaffold 영구배선 — collector 빈슬롯 web brief 자동 산출 2026-06-29**): 수집 시점(handoff v2 카드 producer = raw 생존 지점)에서 `card_scaffold.assemble_web_brief` 로 **빈슬롯 `grm-web-card/v1` 브리프를 결정론 산출**한다 — 1회용 파서·수기 fixture 제거(웹이관 발행 레이어의 진짜 잔여 작업). 신설: 순수 `build_web_brief_payload_v2(rows,run_date,window_days)`(handoff 와 동일 cards·brief_meta) + `emit_web_brief_file`(data 관례 `indent=1`·LF·후행개행) + 플래그 `_enable_web_brief_emit`/`resolve_web_brief_dir`. emit 은 `emit_routine_handoff` v2 분기 내부에서만(raw 생존)·**비차단**(실패해도 handoff 계속). 워크플로(Option A): job-level env `ENABLE_WEB_BRIEF_EMIT`/`GRM_WEB_BRIEF_DIR` + `brief_web_*.json` **artifact 업로드**(사람이 `web/data/briefs/` 커밋 = **D5 라이브 게이트 보존·무인 라이브 0**). `tests/test_web_brief_emit.py`(18: 플래그·schema·빈슬롯·코드 verbatim·결정론·실 producer 경로 동치·파일 LF/후행개행·emit 분기/비차단). additive — handoff/markdown/web-card 골든 byte-diff 0, **777 green**. 직전 `v1.48` 웹이관 P1. |
| 문서 버전 | `v1.49` (**§1-B scaffold 영구배선** — collector emit `build_web_brief_payload_v2`/`emit_web_brief_file`, artifact 배선·D5 보존. 상세 §4.1 트리·§4.2 구성요소 변경이력). 직전 `v1.48` 웹이관 P1(web-card JSON 분리). |
| 최종 수정일 | 2026-06-29 (§1-B scaffold 영구배선 — collector 빈슬롯 web brief emit) |
