...
| 최종 수정일 | 2026-06-04 |
| 현재 상태 | 매일 수집/Notion 적재 동작, GitHub Actions 내부 health check P1 구현, P1 글로벌 3종은 기본 off 상태로 라이브 검증 대기. **제품군 확장(Phase 4) — Notion 속성·preflight 완료. 라이브 적재 검증에서 한국어 정제/주사제 분류 갭 발견·수정(56 테스트 green), 현재 🔴 HOLD(수정분 재검증 전 활성·머지 차단). `ENABLE_MODALITY_TAG` off 유지.** Routine 프롬프트는 v15.8 로 개선(다음 라이브 Routine에서 검증 예정) |
...
| P4-검증 | wiring·Notion 속성·preflight 완료. **라이브 적재 검증(MFDS 55건)에서 한국어 정제/주사제 분류 갭 발견 → 수정 완료(제품명 접미사·한국어 생물 원료), 56 테스트 green.** 남은 것: **수정분 재검증**(같은 B/C 절차로 MFDS 실적재 후 Modality 분포 재확인) → 이상 없으면 `ENABLE_MODALITY_TAG=true` → 1~2주 발행 검증 후 `feature/multi-modality` 머지 | 🔴 HOLD — 한국어 수정 재검증 대기(활성·머지 차단) |
