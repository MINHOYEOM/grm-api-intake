...
- **K2 착수 (🟡 2026-06-05, 별도 Claude Code 세션).** 순서: M0 노이즈필터 → K2-prep(page_id 로 raw fetch) → K2(`build_card_scaffold()` 순수함수 + golden 테스트) → handoff v2. **단계 게이트**(각 단계 끝 테스트 green + 사람 GO/HOLD), scaffold 저장위치 = handoff JSON 본문 포함(additive v2)로 결정.
