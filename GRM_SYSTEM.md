...
- **웹 발행 조립:** Routine이 만든 델타(슬롯만)를 빈슬롯 스캐폴드(전 수집 카드)와 `assemble_publish_brief.py`가 합쳐 **채택분만** 남기고 메타(기관·카테고리·커버리지)를 재계산합니다. "수집 89 · 카드 61" 같은 표기의 근거. 이 단계는 **발행 차단 게이트 3종**을 겸합니다 — 유령 카드(다른 run 스캐폴드)·빈 필수 슬롯·**거짓 부재 서술**(원문을 확보한 카드가 "위반내용이 원문에 없다"고 주장하면 차단, 2026-07-20 사고 대응). 또한 조립 시점에 원문(deep 델타의 `source_text`)에서 483 관찰·**WL 위반항목**을 재추출합니다 — 스캐폴드는 수집 시점 파서로 굳으므로, 파서를 고치거나 새 상세 슬롯이 생겨도 그 주 스캐폴드는 낡은 채로 남기 때문입니다.
