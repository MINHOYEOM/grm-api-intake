...
## ★★★왜 그룹마다 따로 묻는가 (2026-09-05 수리 — 073 이 방문 수까지 깎고 있었다)
`rumPageloadEventsAdaptiveGroups` 의 Adaptive 는 **쿼리 하나를 통째로 보고** 표본
데이터셋을 고른다(ABR). 073 에서 착지 경로(`requestPath`, 사이트가 4천 쪽이라 차원
기수가 크다)를 **같은 쿼리에** 세 번째 그룹으로 붙인 순간, 방문·리퍼러까지 같은
표본 데이터셋으로 내려가 **전 값이 10 의 배수**가 됐다.

실측 경계(2026-09-05 조사): 9/1 까지 리퍼러에 12·6·2·1 같은 정확값이 남아 있고
9/2 14:09 KST 실행(=073 병합 직후 첫 실행)부터 정확값이 0 건. 같은 창의 방문 합이
264(대시보드 대조 완료)에서 210 으로 내려앉았다. 073 당시 "경로별만 반올림"으로
판정했는데 **방문 표를 다시 대조하지 않은 오판**이었다.

→ 지금은 **그룹마다 별도 요청**이다. 요청 하나가 싸질수록 ABR 이 정확한 데이터셋을
고를 확률이 올라간다. 경로가 표본으로 내려가더라도 방문·리퍼러는 딸려 내려가지
않는다. 값이 실제로 표본인지는 추측하지 않는다 — `avg { sampleInterval }` 을 같이
받아 행마다 저장하고 화면이 "정확/추정"을 그대로 표시한다.
