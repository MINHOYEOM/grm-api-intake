...
| 문서 메타 | 값 |
|---|---|
| 문서 버전 | `v1.17` (**Keystone K3 G1·G2 머지** — recall 다품목 병합 렌더(card_spec §14 동결, `merge_recall_cards()`+golden `recall_merged`)·`inmemory_raw` main() 와이어링·fork A안(`render_order`+`group_label`, 정렬 단일 진실원 `_ordered_cards_with_groups()`). `keystone-k3` 4커밋 Codex GO·사람 승인·main 머지, **160 green**. `ENABLE_HANDOFF_V2` 여전히 기본 off — 운영 무영향. 다음 G3(v16 프롬프트)·G4(전환). 직전 v1.16: K3 착수·§14 초안) |
| 최종 수정일 | 2026-06-05 |
