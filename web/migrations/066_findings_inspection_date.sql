...
--   * Health Canada Inspection (1,824건) — 이 마이그레이션의 범위 밖이다.
--     ★★[정정 2026-08-28 · mig 069] 여기 처음에 "raw_json 에 실사일이 아예 없다 …
--       수집 결함이고 리포트카드를 다시 받아야 푼다"라고 적었는데 **틀렸다.** raw_json 의
--       키만 보고 단정했고, 그 값이 다른 칸에 가 있는지는 안 봤다.
--       원천을 직접 찔러 확인한 사실: 목록 API 는 `inspectionStartDate` 를 **전 행**에
--       주고(표본 1,528/1,528) 수집기가 이미 정확히 읽고 있다 — 다만 그 값을
--       `published_date` 에 넣는다(`collect_hc_inspection_backfill.py`). 즉 결손이 아니라
--       **표기**가 틀린 것이었고(실사일에 "공개"라고 적고 있었다), mig 069 가 고쳤다.
--       `inspectionEndDate` 는 전 행 null 이라 캐나다만 시작일이다.
--     ☞ 이 줄을 고치는 이유: 틀린 안내문은 없는 것보다 나쁘다. 그대로 뒀으면 다음 사람이
--       "리포트카드 재수집"이라는 하지 않아도 될 일에 착수했을 것이다.
--
-- ── 키를 소스가 아니라 이름으로 찾는다 ──────────────────────────────────────
