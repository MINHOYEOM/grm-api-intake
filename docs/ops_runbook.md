...
6. **HC 실사 월간 재수집** = 매월 4일 04:23 KST `GRM Health Canada Inspection Deep Backfill` 이 최근 90일 창을 자동 apply(멱등)로 재수집(2026-09-10 신설 — 그 전엔 수동 dispatch 뿐이라 2026-08-10 이후 원장이 얼어붙어 있었다). 스케줄 실패는 이슈 `HC 실사 월간 재수집 실패` 로 뜬다(20h 스로틀, 담당자 배정).
