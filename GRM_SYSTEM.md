...
| 문서 메타 | 값 |
|---|---|
| 문서 버전 | `v1.18` (**Keystone K3 G3 완료 — Routine 프롬프트 v16 동결**(`docs/prompts/GRM_Prompt_v16.md`): 카드별 6슬롯 치환 + A안 조립(render_order/group_label) + 검색 카드 미니 템플릿 + Tier 1 프롬프트 생략(F-1) + Watch 카드/🔮 표 비중복(F-2, card_spec §6 정정 동반) + K4 경계 고지. Codex 조건부 GO → P1(PL-10b `source`+`document_id` 키)·P2×2 반영 후 동결. 운영은 여전히 v15.8+v1(`ENABLE_HANDOFF_V2` off) — 다음 G4 병행 dry-run·전환. 직전 v1.17: G1·G2 머지, 160 green) |
| 최종 수정일 | 2026-06-05 |
