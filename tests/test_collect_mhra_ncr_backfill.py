# -*- coding: utf-8 -*-
"""collect_mhra_ncr_backfill 회귀.

collect_mhra_gmp_ncr(실네트워크) 와 append(PostgREST) 는 둘 다 대역으로 치환한다 —
이 스크립트의 책임은 '수집 결과를 멱등 적재하고 정확히 집계/종료코드화'뿐이라 배관
자체는 각자 모듈 테스트가 이미 커버한다. EU 백필 테스트와 동형.
"""
import io
import os
import sys
...
import collect_mhra_ncr_backfill as mod
...
class TestDryRun(unittest.TestCase):
    def test_dry_run_never_appends(self):
        items = [_Item("Insp GMP A"), _Item("Insp GMP B")]
        appender = mock.Mock()
        report, code = mod.run(
        ...
        self.assertEqual(report.would_append, ["Insp GMP A", "Insp GMP B"])
...
class TestCollectorFailureNotSilent(unittest.TestCase):
    def test_collect_error_exits_2(self):
        ...
        report, code = mod.run(
            ...
            collector=_collector([], err="리스트/네트워크 실패"), appender=appender,
        )
...
class TestMainCredGuard(unittest.TestCase):
    ...
    def test_dry_run_needs_no_creds(self):
        env = {k: v for k, v in os.environ.items()
               if k not in ("SUPABASE_URL", "SUPABASE_SERVICE_ROLE_KEY")}
        with mock.patch.dict(os.environ, env, clear=True), \
                mock.patch.object(mod, "collect_mhra_gmp_ncr", _collector([_Item("A")])):
            buf = io.StringIO()
