...
class WebAdminRenderTest(unittest.TestCase):
    ...
    def test_admin_console_env_gated(self):
        ...
        self.assertIn("워크플로우 관제", h)
        self.assertIn("현재 운영 상태 확인 중", h)
        self.assertIn("관제 상태 읽는 법", h)
        self.assertIn("조치 필요 / 운영 경고", h)
        self.assertIn("새 규제 소식이 수집되어 검증, 배포, 발송까지 이어지는 운영 흐름", h)
        ...
        self.assertIn("필요할 때만 수동 복구 실행", h)
