...
def _from_mhra_gmp_ncr(
    raw_signal: dict[str, Any],
    raw: dict[str, Any],
    row: dict[str, Any],
) -> list[dict[str, Any]]:
    """MHRA GMDP 비준수(SoNC) → 1 레코드 1 finding. EU NCR 추출기의 영국판.

    게이트 = raw.ncr_nature(위반내용 원문·수집기가 상세 페이지에서 결정론 추출). finding_text
    = 위반내용 + 당국조치(둘 다 verbatim). evidence_url = 세션 독립 상세 페이지(mhra_detail_url·
    영속), 부재 시 MHRA 비준수 필터 폴백. `grm_classify_483_scope` 우회는 자동(트리거가
    source='FDA 483'/'FDA Warning Letter' 만 분기 → MHRA NCR 은 scope_status 기본 'ok').
    """
    nature = _compact(raw.get("ncr_nature"))
    if not nature:
        return []
    ...
    if action:
        finding_text = f"{nature}\n\nAction taken/proposed by the NCA: {action}"
    evidence_url = _evidence_url(
        raw_signal, raw, "mhra_detail_url", "mhra_search_url",
        fallback=MHRA_GMP_NCR_SEARCH_URL)
    return [gf.finding_from_raw_signal(
        raw_signal,
        finding_text=finding_text,
        ordinal=1,
...
# 공백을 전부 지운 뒤 비교한다.
# 표의 **칸 이름**(열 머리말·분야·중대도)과 결재란 부서명. ★이건 열거형 라벨이지
# 지적 '내용'이 아니다 — 표가 비어 있으면 라벨만 줄줄이 남아 내용처럼 보인다.
# 실측 반례: "지적(보완)사항 분류 : 분야 구분 근거 법령 지적(보완)사항 요약 품질경영
# 품질경영 시설장비 - 2 - 원자재 z 시험실 제조 의 료 제 품 안 전 과"
_GMP_LABEL_TOKENS = frozenset((
    # 열 머리말
    "분야", "구분", "근거", "법령", "근거법령", "요약", "분류", "비고", "결과",
    "평가결과", "실사결과", "지적보완사항", "지적사항", "있음", "없음", "건", "및",
    # 중대도(PIC/S 구분)
    "기타", "중요", "중대", "other", "major", "critical",
    # GMP 감시 분야 6종 + 실측에 나온 변형
    "품질경영", "품질", "시설장비", "시설", "장비", "제조", "시험실", "원자재",
    "포장표시", "허가관리", "문서관리", "안정성",
    # 결재란 부서명
    "한약정책과", "의료제품안전과", "의약품관리과", "의약품정책과", "의약품품질과",
    "바이오의약품정책과", "바이오의약품품질과", "화장품정책과", "마약정책과",
))
# 정형문·라벨·기호·숫자를 걷어낸 뒤 남아야 하는 최소 실질 글자 수.
_GMP_MIN_SUBSTANTIVE_CHARS = 12


def _gmp_substantive_residual(text: str) -> str:
    """라벨·서식·기호·숫자를 걷어내고 남은 실질 문자열(판정 전용·부작용 없음).

    ★**토큰 단위**로 지운다. 공백을 먼저 뭉개고 부분문자열로 지우면 '제조'가
    '제조기록서'를, '품질'이 '품질관리기록서'를 갉아먹어 진짜 지적까지 깎인다.
    어절이 라벨과 **정확히 같을 때만** 버리므로 '제조기록서'·'품질에'는 살아남는다.

    결재란 부서명은 세로쓰기라 '의 료 제 품 안 전 과' 로 한 글자씩 떨어져 나온다 →
    한 글자짜리 어절은 내용일 수 없으므로 함께 버린다.
    """
    out: list[str] = []
    for token in re.split(r"\s+", text or ""):
        word = re.sub(r"[^0-9A-Za-z가-힣]", "", token)
        if not word or word.isdigit():
            continue
        if len(word) == 1:  # 세로쓰기 결재란 조각·글머리표 잔해
            continue
        if word.lower() in _GMP_LABEL_TOKENS:
            continue
        if re.fullmatch(r"별표\d*(?:의\d+)?|제[0-9의]+호|\d+(?:\.\d+)*호?", word):
            continue
        out.append(word)
    joined = "".join(out)
    # 붙어 있는 정형문(공백 없이 한 어절로 추출되는 경우) 마지막 청소.
    for phrase in ("지적보완사항분류", "지적보완사항요약", "지적사항요약", "지적사항분류",
                   "분야구분근거법령", "근거법령", "평가결과"):
        joined = joined.replace(phrase, "")
    return re.sub(r"[^A-Za-z가-힣]", "", joined)
