...
| 문서 메타 | 값 |
|---|---|
| 최신 변경 | `v1.50` (**뉴스레터 구독·발송·측정 트랙 착수 — 회원 시스템 없이 2026-06-30**): 주간 브리프를 정기 이메일로 발송하고 클릭으로 관심 주제를 측정해 향후 세그먼트 분할 근거를 모은다. **회원가입/로그인 없이 뉴스레터 구독**(이메일+더블옵트인+수신거부)만 — 명단·옵트인·발송·클릭추적은 관리형 SaaS 위임(구독자 PII=SaaS 소유·Notion 비복제). **티저 메일+웹 브리프 링크**(이메일 전용 풀카드 X — 디자인 동결·무변형·메일 CSS 회피·클릭분석 단순화). **T1.1 정적 구독 폼 완료(코드)**: `render.py` env-param `GRM_NEWSLETTER_FORM_ACTION`(`_safe_url` 스킴가드) → `base.html` 푸터 위 env-gated 밴드(이메일 1칸·`method=post`·SaaS endpoint 직접 POST·회원/PII 0·정적$0·런타임 서버 0) + `grm.css` `.subscribe`/`.sub-form`(토큰 재사용·§4 한글안전). 미설정(기본·테스트)=폼 미출력 → 전 페이지 골든 byte-diff 0, `test_render.py::test_newsletter_form_conditional`(+1, **786 green**: off=부재·on=action/email-only/PII 0/한글안전·비http 스킴 fail-safe). branch `feat/newsletter-subscribe-form-t1-2026-06-30`·push/머지 사람 게이트. **T1.3 발송 워크플로**(`grm-newsletter-send.yml` 게이트 4겹: 발행검증·링크체크·멱등·사람승인)·SaaS 셋업(사람)·**T2 세그먼트**(클릭 조인 `web/analytics/`·주제 채널 render 변형) 후속. 상세 §5.3·지시문 `GRM_CC지시문_뉴스레터구독발송측정_2026-06-30.md`. 직전 `v1.49` §1-B scaffold 영구배선(상세 §4.2 변경이력). |
| 문서 버전 | `v1.50` (**뉴스레터 구독·발송·측정 트랙 착수** — T1.1 정적 구독 폼: `render.py` env-param `GRM_NEWSLETTER_FORM_ACTION` env-gated 밴드·정적$0·전 페이지 골든 byte-diff 0. 상세 §5.3·로드맵 변경이력). 직전 `v1.49` §1-B scaffold 영구배선. |
| 최종 수정일 | 2026-06-30 (뉴스레터 구독·발송·측정 트랙 착수 — T1.1 정적 구독 폼) |
...
- **이메일 발송·측정 (회원 시스템 없이 — 2026-06-30 착수, P3 이메일 진입):** 주간 브리프를 정기 이메일로 발송하고 클릭으로 관심 주제를 측정해 향후 세그먼트 분할 근거를 모은다. 종전 §5.2 "구독자 필터 제외"의 **회원 시스템 없는 경량 경로** — 필요한 건 회원제가 아니라 **뉴스레터 구독**(이메일+더블옵트인+수신거부). 명단·옵트인·수신거부·발송·클릭추적은 **관리형 SaaS 위임**(구독자 PII=SaaS 소유·Notion 비복제), 개인화는 "회원"이 아니라 "주제 채널"로(나중). **티저 메일+웹 브리프 링크**(이메일 전용 풀카드 X — 디자인 동결 v4·무변형·정적$0 보존, 메일 CSS 호환·클릭분석 단순화).
  ...
  - **T1.3 발송 워크플로(후속):** 신규 `grm-newsletter-send.yml`(`grm-web-deploy.yml` 패턴·수집/배포와 별도 파일·별도 트리거·최소권한·D8). 발송 전 **게이트 4겹**: ① 발행 검증 통과(`verify_published_brief`/`run_publish_gate`) ② 링크체크 재사용(`web/linkcheck.py` broken 시 발송 보류) ③ 멱등(`publish_date` 키=재발송 0) ④ 사람 승인(`workflow_dispatch`/`environment: production`=무인 발송 0, D5). 본문=티저(tldr + "전체 보기" + 섹션 앵커 링크) + 면책 캐논. From=자체 발송 도메인(SPF·DKIM·DMARC, 사람). 클릭 추적은 SaaS 가 발송 시점 래핑 → 우리 카드 원문/공식 URL 불변(provenance·무변형 보존).
