...
--
-- ─ ★2026-08-31 개정 — "디렉터리 금지" 조항을 좁힌다(가나다순 색인 신설) ──────────
--   위 "디렉터리(목록 열람) 페이지를 만들지 않는다" 조항은 **삭제하지 않는다** — 그
--   문장이 막던 것(순위·비교·엄격도 추론)은 지금도 그대로 금지다. 다만 그 문장이 실제로
--   막으려던 대상을 다시 짚는다: 금지된 것은 "목록의 존재"가 아니라 "사람을 서열화하는
--   것"이었다.
--
--   사용자 실측 지적 — `/findings/inspector/` 조회 화면이 빈 검색창이라 **이름을 이미
--   알아야만** 쓸 수 있었다(사이트 안에 실사관 이름을 알 방법이 없었다). 사용자 결정을
--   그대로 옮긴다: "순위를 매기지 말라는 거고, 가나다 순으로 색인하면 됨. 그래야 찾기로
--   없을 때 찾을 수 있음." — 즉 개정 사유는 "디렉터리를 허용해도 되는가"가 아니라
--   "이 디렉터리가 서열화를 만드는가"이고, 답이 "만들지 않는다"이면 그 형태(이름순 색인)
--   는 애초에 금지 대상이 아니었다.
--
--   경계(웹 레이어 — web/assets/inspector.js·web/templates/inspector.html·
--   web/tests/test_render.py 범위 가드가 강제):
--     여전히 금지 — 실사관 간 순위·비교, 엄격도/성향 해석, **건수 표기나 건수 기준
--       정렬을 통한 사실상의 랭킹**.
--     새로 허용 — **이름순 색인 하나**: 정렬 키는 `display_name` 오름차순뿐, 건수는
--       어디에도 표기하지 않는다, 코호트(문서 5건 이상)로 한정된다, `/findings/inspector/`
--       조회 화면(?key= 없는 상태) 안에서만 산다 — 별도 라우트를 새로 만들지 않는다.
--
--   이 파일(RPC)은 이 개정으로 바뀌지 않는다 — `findings_inspector_index()` 는 037 당시
--   그대로 코호트 키 목록만 돌려주고, 이름순으로 훑어보이게 하는 것은 그 응답을 소비하는
--   클라이언트 쪽 관심사다.
--
-- ─ 정체성 키 ───────────────────────────────────────────────────────────────
