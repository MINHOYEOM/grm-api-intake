...
- ✅ **반영 완료(9건 중 7건):** PL-1·3·6(Claude/v15.6) · PL-2·4·5·9(Codex/코드+라이브 스키마). 코드 py_compile·diff 통과, 30/90일 dry-run 정상. **stage/commit 미실시.**
- 🔲 **잔여:** PL-7(수집기 부재 소스 — 품목허가 변경·DMF 등, A축 확장) · PL-8(보조출처 403). 둘 다 별도 트랙, 배포와 무관.
- ⛔ **배포 게이트(운영 반영 전):** ①MFDS 조회 실측(Claude (B)경로 ✓, 프로덕션 계정 재확인) ②Self-Check 코드+출력 동시 커밋 ③국내 섹션 실렌더 1회 확인. → 게이트는 `GRM_Prompt_v15.6.md` C절 참조.
