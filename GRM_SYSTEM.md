...
- **파셋·대시보드 통계 정합성(`025_findings_stats_review_status.sql`):** `/findings/`의 파셋 셀렉트 `(N)` 카운트·대시보드(스탯/카테고리/월별/업체) 숫자는 **무필터·무검색 상태에서만** `findings_stats` RPC(007, 소스·증거등급·발행월·카테고리·업체 Top 30 + `by_review_status`)의 전역 truth로 채운다 — 클라이언트가 실제로 들고 있는 `ROWS`는 최신순 최초 1,000행뿐(점진 청크 로드)이라 그대로 세면 전역 비율이 왜곡되기 때문이다(예: 최신순 정렬 상단에 몰린 소스는 로드분에 100% 잡혀 실제 비중보다 훨씬 크게 보인다). 발행월 옵션도 같은 이유로 ROWS가 아니라 RPC `by_month` 전역 값으로 채운다. 필터·검색이 하나라도 활성이면 파셋 옵션·대시보드는 숫자를 숨기고 라벨만 남긴다(로드분 부분집합 건수를 사실값처럼 보여주지 않는다는 원칙 — 툴팁으로 "로드분 기준"이라 얼버무리지 않는다). 결과 영역에는 서버 obs 청크가 소진되기 전까지 "데이터를 추가로 불러오는 동안 결과가 갱신됩니다" 문구가 상시 노출되고, 전량 로드가 끝나면 사라진다. 오래된순·업체명순 정렬은 전량 로드 전에는 전역 정렬을 보장할 수 없어 비활성화되며(전량 로드 후 자동 재활성화), 최신순(기본)은 서버가 `published_date desc, finding_id asc`로 내려주므로 부분 로드 상태에서도 상단이 항상 전역 진실이라 영향받지 않는다. `by_review_status`가 없는 라이브 DB(025 미적용)에서도 안전하게 감지해 기존 로드 기준 폴백으로 조용히 동작한다.
- **임베딩 저장층(S2, 구현 완료·웹 공개 중단):** `findings_embed_service.py`(LLM 미관여 순수 스크립트)가 공개 게이트 통과분을 로컬 모델(`intfloat/multilingual-e5-small`, 384차원, GitHub Actions CPU)로 임베딩해 `finding_embeddings`(019, embedding_version·finding_id 복합 PK)에 upsert한다. 아이템-투-아이템 대칭 유사도라 양쪽 모두 `"query: "` prefix(019 주석·서비스가 함께 고정). embed_input B(483 전용)는 `raw_signals.raw_json`의 Observation deficiency+detail 재조합을 시도하고 모호/무매치/detail없음이면 A(finding_text 단독)로 결정론 폴백한다. `findings_similar_by_id` RPC(019)가 `embedding_config.active_version`과 일치하는 벡터만 서빙하지만, **A/B 평가 실측(2026-07-15, 평가셋 40건·블라인드 pooled 판정·부트스트랩 95% CI)에서 S1(렉시컬) 대비 유의한 개선을 입증하지 못해(P@5/nDCG 동률 또는 오히려 열세 — 공개 483 의 59.4%가 CFR 조항을 옮긴 동일 문구라 렉시컬이 이미 정답을 찾는다) §4.3 공개 게이트를 불통과, S2 웹 공개는 중단됐다.** 저장층·서비스 자체는 유지하되 `findings_similar_by_id` 는 호출하지 않는다(inert) — 웹의 "유사 사례" 기능은 021(S1 렉시컬) 이 전담한다. **F-04(수리 완료):** `fetch_existing_embeddings`가 `text_sha256`뿐 아니라 `model`도 함께 읽어(`finding_id -> (text_sha256, model)`), 재임베딩 판정을 `text_sha256 일치 AND model 일치`로 강화했다 — `resolve_model_revision` 호출도 스킵 판정보다 먼저 수행하도록 순서를 옮겼다(모델 로드 자체는 종전대로 실제 임베딩 직전에만). 리포트에 `revision_changed`(텍스트는 같으나 model 이 달라 재임베딩된 행 수) 카운터를 추가했다. **F-05(수리 완료):** `grm-findings-embed.yml`의 action 3종(`checkout`/`setup-python`/`cache`)을 전부 full commit SHA로 pin했고, `requirements-embed.txt`를 floating range 대신 정확 버전 고정(torch/sentence-transformers/transformers/huggingface_hub/numpy)으로 교체했다(잔여 과제: 전이 의존성 전체의 hash-lock은 아직 없음). **F-09(잔존 무해):** 과거 실행에서 남은 비공개 벡터 358×2건이 `finding_embeddings`에 남아 있으나 §4.3 공개 게이트가 걸러내 웹 서빙에는 영향이 없다.
