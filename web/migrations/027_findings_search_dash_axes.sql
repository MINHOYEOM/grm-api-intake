...
--
-- supersede 체인: 026_findings_search.sql(원본 정의) → 이 파일(027)이 findings_search 를
--   create or replace 로 supersede 한다. findings_document 는 026 정의 그대로 유효하며
--   이 파일은 재선언하지 않는다.
--
...
comment on function public.findings_search(text, text, text, text, text, text, text, text, int, int) is
  ...
  '025 §⑤ 이행.';
-- ---------------------------------------------------------------------------
-- (B) findings_document — 026 정의 그대로 유효(이 파일은 재선언하지 않는다)
--     ※ 2026-07-16 후속: 이후 028 이 findings_document 를 supersede 했다(투영 3종 복원).
--       프로덕션 현행 정의는 028 을 참조하라 — 아래 원문은 027 작성 시점 기준 기록이다.
-- ---------------------------------------------------------------------------
-- 027 은 findings_search 의 반환 계약만 확장한다. 딥링크 해석은 대시보드 축과 무관하므로
-- 026 의 정의를 건드릴 이유가 없다 — 불필요한 재선언은 diff 를 부풀리고 "무엇이 바뀌었나"를
-- 흐린다. 프로덕션 현행 정의는 026 을 참조하라.
...
-- 성립하도록 findings_search 의 grant 를 재선언한다(멱등). findings_document 는 이 파일이
-- 만들지 않으므로 여기서 grant 하지 않는다(026 이 담당).
