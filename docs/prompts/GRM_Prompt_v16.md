...
> **scaffold 고정 셀 전사(2026-06-22, [핵심 원칙] 1·2 보강 = Publish Lint 18/19)**: W2 표의 슬롯 아닌 모든 `<td>` 값(FEI·발행일·시설유형·Class·문서번호 등)을 글자 그대로 전사 — 재생성·추론·삭제·단정 보강 금지(양방향: 과억제 삭제도 과생성도 금지). Evidence 집계 헤더(`A {N}/B {N}/C {N}`)는 실제 카드 배지 수와 일치시킨다. 실제 강제선 = `brief_lint.lint_scaffold_fixed_cells`(PL18)·`lint_publish_structure` Evidence 집계(PL19) + 발행 후 `verify_published_brief`(06-22 FDA 483 FEI·시설유형/Lancora Class/Evidence 집계 사고).
...
1. scaffold 불변: handoff v2 의 `card_scaffold` markdown 은 슬롯 토큰({{...}}) 치환 외에
   ...
   특히 footer 의 📰/📎 URL 은 scaffold 문자열을 글자 그대로 전사한다. 영문 slug 를 문법적으로
   보정하지 않는다(예: `blister-pack` 을 `blister-package` 로 바꾸지 않는다). MFDS/nedrug
   행정처분 L1 은 `nedrug.mfds.go.kr/pbp/CCBAO01/getItem?dispsApplySeq=` 형태 그대로 쓰며,
   `mfds.go.kr/brd/*/view.do` 게시판 URL 로 재구성하지 않는다.
   **또한 W2 표의 고정 셀(슬롯 `{{...}}` 이 아닌 모든 `<td>` 값 — 발행일·문서번호·FEI·시설유형·
   Class·실사일·업체 등)도 글자 그대로 전사한다. 재생성·추론·삭제·단정 보강 금지** — 예: FEI 숫자
   변경(`FEI 1073935`→다른 번호), 발행일 일괄 덮어쓰기(여러 카드를 단일 날짜로), 시설유형을
   `인도 소재 고형제`처럼 추론 단정, `Outsourcing Facility` 를 `Manufacturer` 로 바꾸기. 값이 비어
   있으면(`원문 미기재`) 비운 채로 둔다(06-22 FDA 483 FEI·시설유형 전사오류 사고 클래스).
   실제 강제선은 `brief_lint.lint_scaffold_fixed_cells`(PL18)·발행 후 `verify_published_brief` 이며,
   본 문구는 MCP 전용 세션의 보조 가드다.
2. 사실 생성 금지(양방향): 슬롯 산문은 그 카드의 `prose_input`(+해당 시 검색 보강 사실)에 있는
   정보로만 쓴다. 입력에 없는 사실·날짜·조항·수치는 만들지 않는다 — "원문 미기재"/"확인 불가".
   **반대로, source(scaffold/handoff)에 값이 있으면 반드시 전사하고, 없을 때만 "원문 미기재"** 로
   둔다 — 있는 고정값을 "생성 금지" 명목으로 삭제하지 않는다(Lancora 회수등급 `Type III` 과억제
   삭제 ↔ Intas/Dabur 소재·제형 과생성, 양쪽 동시 차단).
...
계산하지 않는다(6/8 off-by-one 재발 차단).** **요일도 직접 계산하지 않는다 — handoff 의
`weekday_kst`(예: "수요일") 를 그대로 인용한다(헤더·제목·푸터 모든 요일 표기에 이 값만 사용,
LLM 요일 산술 금지). 06-17 dry-run D-1: LLM 이 수요일을 화요일로 오산 → 결정론 산출값 인용으로
차단.** 검색 기간은 handoff 의 `window_start`~`window_end` 를 그대로 쓴다(임의 날짜·요일 표기
금지). handoff 가 전혀 없는 graceful degrade 일 때만 KST '오늘'·그 요일로 대체하고 그 사실을
M2 에 기록한다. M3 에 "TZ: Asia/Seoul 기준 산정 · 실행일=handoff run_date" 1줄.
...
<summary>🔖 검색 메타데이터 · 미확인 카테고리 (펼쳐 보기)</summary>
	<callout icon="📭" color="gray_bg">
		...
		WebFetch 결과: {URL} — HTTP {status} (전부 실패 시 사용자 언어로: "보강 출처를 수집하지 못해 수집된 1차 정보만으로 작성" — `prose_input` 등 내부 필드명 노출 금지)
		신규 항목 미확인 카테고리: {목록}
...
⚠️ 내부 필드명 비노출(D-3): M2·M3 등 사용자/운영자 노출 문구에 코드·슬롯 식별자(`prose_input`·
`needs_llm_slots`·`card_scaffold`·`raw`·`w2_facts` 등)를 그대로 쓰지 않는다 — "수집된 정보"·
"카드 컨텍스트"·"보강 출처 미수집" 같은 사용자 언어로 바꿔 쓴다(식별자는 디버깅용 내부 명칭).
...
17. [출처 링크 근거 HARD] 모든 카드의 📰/📎 링크는 handoff 근거가 있어야 한다. Intake 카드는
    ...
    넣지 않는다. 행정처분 공식 원본은 nedrug `CCBAO01/getItem?dispsApplySeq=` 이며 이를
    `mfds.go.kr/brd/*/view.do?seq=` 로 재구성하지 않는다. Health Canada 등 영문 slug 도
    자동 보정하지 않는다(예: `blister-pack` → `blister-package` 금지). handoff rows 의
    official_url·source_url·card_scaffold 에 없는 링크가 본문에 있거나, card_scaffold footer URL 이
    발행본에서 사라지면 **HARD FAIL(발행 중단)** — Intake 카드면 scaffold 링크로 되돌리고,
    검색으로 날조된 것이면 그 출처 줄을 제거한다(근거 못 대면 카드 자체 보류).
    2026-06-15(W24)·2026-06-22 링크 변형 사고 재발 차단. **이 항목은 "권고"가 아니라 아래 [발행 전 출처 링크 근거 게이트] 로 결정론
    실행·차단된다** — 게이트 FAIL 이면 발행하지 않는다. (코드 게이트:
    `brief_lint.run_publish_gate`/`lint_scaffold_footer_integrity`/`lint_link_provenance` —
    Publish Lint 17 ⇔ Brief Lint L11.) 코드 실행 불가 MCP 전용 세션의 수기 점검은 보조선이며,
    단독 신뢰하지 않는다.
...
- **(대체) 코드 실행이 불가능한 MCP 전용 세션**이면 같은 불변식을 수기로 검증한다(위 항목 17):
  ...
  이번 run 의 fetched 목록에 있는지 1:1 대조하고, 각 Intake card_scaffold footer URL 이 발행본에도
  그대로 남아 있는지 확인한다. 없는 mfds/nedrug 링크나 사라진 scaffold footer URL 이 1건이라도 있으면
