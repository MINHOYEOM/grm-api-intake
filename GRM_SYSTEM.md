...
| 문서 메타 | 값 |
|---|---|
| 문서 버전 | `v1.14` (Keystone K2 B~D 코드 완료·머지 대기 — `card_scaffold.py`(build_card_scaffold+golden)·K2-prep raw fetch·handoff v2(`ENABLE_HANDOFF_V2` off), 전체 133 green. Stage A(M0+P1) origin/main 머지 완료. 직전 v1.13: M0 부서 게이트. v1.12: 바이오 1단계 활성) |
| 최종 수정일 | 2026-06-05 |
| 현재 상태 | 매일 수집/Notion 적재 동작, GitHub Actions 내부 health check P1 구현. **바이오 소스 1단계: Phase 3 P1 글로벌 3종(ICH·WHO·HC) 라이브 검증 통과(dry-run + 실적재 207건 0실패)·운영 활성(`ENABLE_ICH/WHO/HC=true`)·`feature/biologic-sources` → `main` 머지.** 라이브 검증 중 발견된 GAP-1(`MODALITY_BIOLOGIC_TERMS` 에 `biotechnological` 미포함 → ICH Q5A-E 미분류) 1줄 수정 + 회귀 1건 추가(82/82 green). Notion `Source` Select 옵션 사전등록(ICH·WHO·Health Canada — 1095일 source-scoped dedup 의 `select.equals` 가 미존재 옵션에서 400 반환하는 Notion 거동 회피). Routine 프롬프트 v15.8 다음 라이브에서 검증 예정 |
| Active phase | 바이오 1단계 활성 — 1~2주 관찰(Biologic 칸 누적 증가·세 수집기 health·발행 브리프) + Phase 4 운영 관찰 + Routine 프롬프트 v15.8 검증 + **Keystone K2 진행(Python 카드 조립기, 발행/조립 계층 — 수집과 직교)** + (필요 시 2단계 FDA CBER guidances 신규 수집기 트랙) |
| 주요 enabled flags | 운영 기본: `ENABLE_MFDS/RECALL/ADMIN/GMP_INSPECTION=true`, `ENABLE_MODALITY_TAG=true`(2026-06-04 활성), **`ENABLE_ICH/WHO/HC=true`(2026-06-05 활성)**, `ENABLE_SCRAPE/MOLEG_API=false` |
| 기준 시스템 버전 | `origin/main` `89e744b`(Phase 4 머지) + **`feature/biologic-sources` 브랜치(바이오 1단계: `MODALITY_BIOLOGIC_TERMS` 에 `biotechnological` 추가·test_modality 회귀 1건, Source 옵션 ICH/WHO/Health Canada 사전등록)** · Routine 프롬프트 `v15.8` |
...
│  ├─ test_card_scaffold.py           # [K2] build_card_scaffold golden(유형별 5종+페이지) 바이트 동결·금지문법 부재
│  ├─ test_handoff_v2.py              # [K2] handoff v2 플래그·additive·raw 미포함·children 분할·v1 스냅샷 잠금
│  └─ golden/                         # [K2] 카드 골격 golden fixture(input/expected.md/json)+v1 스냅샷
│
...
수집과 카드 생성은 **동작하는 상태** 이며, 2026-06-01 라이브 실행에서 국내 섹션·소재국 fallback·지적사항 요약·듀얼 링크가 정상 렌더됨을 확인했습니다. 카드 포맷 표준 자체는 `v15.6.2` 에서 확정되었습니다. **P0(국내 MFDS) 신호 누락·강등 보강**·**Phase 4(제품군 확장)**·**Phase 3 P1(글로벌 ICH·WHO·HC) = 바이오 1단계** 모두 라이브 검증 통과·운영 활성 상태이며 머지 완료입니다. 이번 바이오 1단계 활성으로 모집단에 바이오 비중이 높은 채널(WHO PQ·ICH Q-series·HC drug recall)이 일상 수집에 합류했습니다. 남은 것은 **카드 내용 완성도 다듬기 + 바이오 1단계 1~2주 관찰(Biologic 칸 누적·세 수집기 health·발행 브리프) + Routine 프롬프트 v15.8 다음 라이브 검증 + (조건부) 바이오 2단계 = FDA CBER guidances 신규 수집기 + P2 Codex heartbeat 요약 설계**입니다.
