...
├─ library_collect_*.py            # [트랙 C] 자료실 소스별 수집기 플러그인(ema·eu_gmp·fda_guidance·health_canada·pics·pmda·who — 파일 추가만으로 자동 발견)
...
│  ├─ data/  (briefs·deltas·library/ 9카탈로그 400건 + library_updates.json 변경 이력·glossary.json 226어 + glossary_cases.json 사례연결 정본(items 177·excluded 49)·guide_content.md·quiz_bank.json 33문항·announcements/ 서비스 업데이트 안내 — 전 링크 실검증 커밋 데이터)  ·  partials/  ·  tests/  (render 골든, trends.expected.html 포함)
├─ translations/
