# GRM Brief Lint — 발행 후 검증 실행프롬프트 (v1.3)
...
L11 출처 링크 근거(provenance, HARD FAIL — 2026-06-15 W24 회귀): 모든 카드 📰/📎 링크가
    ...
    **MFDS/nedrug 링크**가 본문에 있으면 FAIL — 특히 `mfds.go.kr/brd/*/view.do?seq=`(보도자료
    ...
    패턴 유추(환각) 의심.
L12 scaffold 고정 셀 전사(provenance, HARD FAIL — 2026-06-22 사고): 각 렌더 카드의 W2 표
    **identity 셀**(FEI 동반 제조소/업체·문서번호·시설유형·Class·제품·업체)이 handoff `card_scaffold`
    값과 **글자 그대로** 일치하는가. FEI 숫자 변경·시설유형 추론 단정(예: `Outsourcing Facility`→
    `인도 소재 고형제`)·회수등급(`Type III`) 삭제는 FAIL. **날짜 셀은 소스별**: FDA 483·MFDS 행정처분의
    발행일/처분일/실사일은 정본이라 verbatim(여러 카드 단일 날짜 덮기·처분일 수집일 오사용 = FAIL),
    FDA WL·FR·ECA·HC 발행일은 수집일 placeholder→WebSearch enrich 가 정상이라 검사 제외(오탐 아님).
    **결정론 게이트 1차**(`brief_lint.lint_scaffold_fixed_cells`=PL18, identity 전 소스 + 483/admin
    날짜·카드영역 한정, 항상 실행)이며, 본 항목은 그 게이트 결과를 인용한다(수기 재대조 금지 — EVAL 만
    잡고 Lint 가 놓치던 비대칭 제거. 06-22 실데이터 FP 0/TP 8).
L13 Evidence 집계 일관성(2026-06-22 사고): 커버리지 헤더의 `Evidence A {N}/B {N}/C {N}` 선언값이
    실제 카드 W1 배지 수와 일치하는가(헤더 A4/B10 인데 배지 A3/B11 면 FAIL). 결정론 게이트
    `brief_lint.lint_publish_structure`=PL19(`--structure` 또는 발행 후 탐지에서 실행) 결과를 인용한다.

[검사 방법]
- 페이지 본문을 한 번 fetch 해 전 카드를 순회하며 L1~L6, L10 을 채점한다.
- Intake DB 는 해당 주 handoff page 1건 + 그 rows 의 Status 만 조회한다 (broad search 금지).
- L11·L12·L13 은 코드 게이트로 **결정론 판정**한다(권고: 수기 대조 대신 실제 실행):
  · 코드 실행 가능 환경: handoff page 본문 JSON 을 `handoff.json`, 페이지 export 를 `brief.md`
    로 저장하고 레포 루트에서
    `python -m brief_lint --handoff handoff.json --published brief.md --verify --structure` 실행.
    exit 1 이면 FAIL(리포트 코드로 항목 매핑: `L17-*`=L11 / `PL18-SCAFFOLD-CELL`=L12 /
    `PL19-EVIDENCE-TALLY`=L13 / `PL14-WEEKDAY`=요일). `--verify` 는 비-MFDS 미근거 링크 live
    verify, `--structure` 는 PL14/PL19 등 구조 검사를 켠다(scaffold footer/셀 무결성 PL18 은
    `--structure` 없이도 항상 실행).
...
[출력 형식 — 채팅 보고 전용]
1. 요약 한 줄: "L1~L13 중 PASS {n} · FAIL {m} · 확인 불가 {k}"
...
| 2026-06-22 | **L12 scaffold 고정 셀 전사 + L13 Evidence 집계 추가**(주간점검 트랙 A·B·D) — EVAL 만 잡고 Lint 가 놓치던 비대칭 제거. W2 표 고정 셀(FEI·발행일·시설유형·Class 등) 전사 무결성 = `lint_scaffold_fixed_cells`(PL18, 항상 실행), Evidence 집계 헤더↔배지 = `lint_publish_structure`(PL19). CLI 에 `--structure` 추가. 결정론 게이트가 1차, 관측은 그 결과 인용(수기 재대조 금지). 발행 후 자동 탐지(`verify_published_brief`)도 PL18/PL19 를 독립 재실행. v1.2→v1.3 |
